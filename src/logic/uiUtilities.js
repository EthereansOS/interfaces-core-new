...
export function retrieveComponentsByReflection(
  contextualRequire,
  key,
  returnElement
) {
...
export function retrieveSavedPath(menuVoices, currentLocationInput) {
  ...
  selectedVoices =
    selectedVoices.length > 0
      ...
      : database.filter(
          (it) =>
            selectedVoices.indexOf(it) === -1 &&
            it !== currentLocation &&
            currentLocation.indexOf(it.split(':')[0]) !== -1 &&
            it.split('/').length === currentLocation.split('/').length
        )
  selectedVoices =
    selectedVoices.length > 0
      ? selectedVoices
    ...
    selectedVoices.length > 0
      ...
      : database.filter(
          (it) =>
            selectedVoices.indexOf(it) === -1 &&
            it !== currentLocation &&
            it.split(':')[0].indexOf(currentLocation) !== -1
        )
...
export function prepareAddToPlugin(
  contextualRequire,
  all,
  name,
  link,
  ...
  return {
    ...
    addToPlugin:
      ({ index }) =>
      ({ addElement }) => {
        retrieveComponentsByReflection(all, 'menuVoice').forEach(
          (it, i) =>
            it.path &&
            addElement('router', {
              ...it,
...
export const OpenSeaContextProvider = ({ children }) => {
  ...
  const seaport = useMemo(() => {
    ...
    return theWeb3 && theWeb3.currentProvider
      ? new OpenSeaSDK(theWeb3.currentProvider, {
...
export async function perform({ setLoading, call, onSuccess }) {
...
export const GlobalModalContextProvider = (props) => {
  ...
  value.show = function show(data) {
    const { onClose, className, timeout, content } = data || {}
    if (content && children) {
      return
    }
    setOnClose(onClose)
  ...
  return (
    <GlobalModalContext.Provider value={value}>
      {children && (
        <div className={style[chosenClassName] || custom[chosenClassName]}>
          {children}
        </div>
      )}
      {props.children}
