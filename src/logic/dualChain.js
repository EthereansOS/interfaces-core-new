  ...
  10: {
    async resolveToken(web3Data, it) {
      ...
      try {
        ...
        var dualTokenAddress =
          context.dualChainTokens[addr] ||
          context.dualChainTokens[chainId][addr]
        dualTokenAddress =
          dualTokenAddress &&
          ((typeof dualTokenAddress).toLowerCase() === 'string'
            ? dualTokenAddress
            : dualTokenAddress[dualChainId])
        if (dualTokenAddress) {
          return setAddress(it, dualTokenAddress)
        }
        dualTokenAddress = await getRawField(
...
export async function lookupAddressDualChain(data) {
  var name
  var { dualChainId, dualChainWeb3, web3, chainId, address } = data

  var theWeb3 = dualChainWeb3 || web3
  var theChainId = dualChainId || chainId

  const ethersProvider = new ethers.providers.Web3Provider(
    theWeb3.currentProvider,
    theChainId
  )
