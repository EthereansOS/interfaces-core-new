...
export async function getDelegationsOfOrganization(
  { context, chainId, web3, account, getGlobalContract, newContract },
  organization
) {
  ...
  var delegationAddresses = await Promise.all(
    list.map((it) =>
      getRawField(
        { provider: delegationsManager.currentProvider },
        it.treasuryManagerAddress,
        'host'
      )
    )
  )
  ...
  for (var i in delegationAddresses) {
    ...
    list[i].delegation = newContract(
      context.SubDAOABI,
      (list[i].delegationAddress = list[i].key = delegationAddress)
    )
  }
...
export async function getAvailableDelegationsManagers(
  {
    context,
    chainId,
    web3,
  ...
  var delegationsManagers = await getDelegationsManagers({
    context,
    chainId,
    web3,
    account,
  ...
  delegationsManagers = delegationsManagers.filter(
    (it) => exceptFor.indexOf(it.delegationsManagerAddress) === -1
  )
  delegationsManagers = lightweight
    ? delegationsManagers
    : await Promise.all(
        delegationsManagers.map(async (it) => ({
          ...it,
          organization: await getOrganizationMetadata(
            { context },
            {
              contract: newContract(context.ISubDAOABI, it.hostAddress),
...
export async function getDelegationsManagers(
  {
    context,
    chainId,
    getGlobalContract,
  ...
  const factoryIndices = getNetworkElement(
    { context, chainId },
    'factoryIndices'
  )

  var indices = Object.entries(factoryIndices)
    .filter(
      (it) => it[0] === 'organization' || it[0] === 'dfo' || it[0] === 'subdao'
    )
    .map((it) => it[1])
  ...
  await Promise.all(
    indices.map(async (it) =>
      address.push(
        ...(
          await blockchainCall(factoryOfFactories.methods.get, it)
        )[1]
      )
    )
  )
  ...
  const COMPONENT_KEY_DELEGATIONS_MANAGER =
    '0x49b87f4ee20613c184485be8eadb46851dd4294a8359f902606085b8be6e7ae6'
  ...
  args = {
    ...
    fromBlock: web3Utils.toHex(
      Math.min(...logs.map((it) => parseInt(it.blockNumber)))
    ),
    toBlock: 'latest',
  }
  ...
  address = await Promise.all(
    address.map((it) =>
      blockchainCall(
        newContract(context.SubDAOABI, it).methods.get,
        COMPONENT_KEY_DELEGATIONS_MANAGER
      )
    )
  )
  ...
  var contracts = address.map((it) =>
    newContract(context.DelegationsManagerABI, it)
  )

  var hosts = await Promise.all(
    contracts.map((it) => blockchainCall(it.methods.host))
  )
  ...
  if (delegation) {
    hosts = await Promise.all(
      hosts.map(async (it) => {
        var data = await blockchainCall(
          it.delegationsManager.methods.exists,
          delegation
        )
        if (data[0]) {
          return {
...
export async function createDelegation(
  ...
  var deployOrganizationDataValue = [
    ...
    voteRules
      ? abi.encode(
          ...
          [
            voteRules.host,
            toDecimals(numberToString(parseFloat(voteRules.quorum)) / 100, 18),
            voteRules.validationBomb,
            voteRules.votePeriod,
            toDecimals(numberToString(parseFloat(voteRules.hardCap)) / 100, 18),
          ]
        )
      : '0x',
  ]
  ...
  var factoryData = await blockchainCall(
    ...
    getNetworkElement({ context, chainId }, 'factoryIndices').delegation
  )
  ...
  var transaction = await blockchainCall(
    factory.methods.deploy,
    deployOrganizationData
  )
  var transactionReceipt = await sendAsync(
  ...
  logs = logs.filter(
    (it) =>
      it.topics[0] === web3Utils.sha3('Deployed(address,address,address,bytes)')
  )[0]
...
export async function finalizeDelegation(
  { context, chainId, newContract, factoryOfFactories },
  delegationAddress,
  host,
  quorum,
  validationBomb,
  ...
  await blockchainCall(
    factory.methods.initializeProposalModels,
    delegationAddress instanceof Array
      ? delegationAddress[0]
      : delegationAddress,
    host,
    quorumPercentage,
...
export async function all(
  { context, newContract, chainId, factoryOfFactories, account, mine },
  metadata,
  organization
) {
  const factoryIndex = getNetworkElement(
    { context, chainId },
    'factoryIndices'
  ).delegation
  ...
  var delegations = logs.map((it) =>
    web3Utils.toChecksumAddress(abi.decode(['address'], it.topics[2])[0])
  )

  try {
    var excluded = context.delegationsToExclucde.map(
      web3Utils.toChecksumAddress
    )
        ...
        try {
          ...
          var host = abi
            .decode(
              ['address'],
              await getRawField(
                { provider: it.currentProvider },
                proposalModels[0].creationRules,
                'value'
              )
            )[0]
            .toString()
          ...
          var initializer = await blockchainCall(it.methods.initializer)
          initializer = await getRawField(
            { provider: it.currentProvider },
            initializer,
            'deployer(address)',
            it.address
...
export async function getDelegation(web3Data, delegationAddress) {
  ...
  var initializer = await blockchainCall(
    delegation.contract.methods.initializer
  )
  initializer = await getRawField(
    { provider: delegation.contract.currentProvider },
    initializer,
    'deployer(address)',
    delegation.address
  )
  ...
  try {
    delegation.host = abi
      .decode(
        ...
        await getRawField(
          { provider: delegation.contract.currentProvider },
          delegation.proposalModels[0].creationRules,
          'value'
        )
      )[0]
      .toString()
...
export async function attachToOrganization(
  {
    context,
    chainId,
    web3,
  ...
  var attachInsurance = await blockchainCall(
    selected.delegationsManager.methods.attachInsurance
  )

  var token = await blockchainCall(
    selected.delegationsManager.methods.supportedToken
  )
  token = await loadToken(
    { context, chainId, web3, account, newContract, getGlobalContract },
    token[0],
    token[1]
  )

  var paidFor = await blockchainCall(
    selected.delegationsManager.methods.paidFor,
    element.address,
    account
  )
  paidFor = paidFor[0]
  ...
  var additionalUri = await uploadMetadata(
  ...
  await propose(
    ...
    abi.encode(
      ['string', 'address'],
      [additionalUri, delegationsManagerAddress]
    ),
    true
  )
}

export async function setDelegationMetadata(
  { context, ipfsHttpClient },
  element,
  additionalMetadata,
  metadata
  ...
  await propose(
    ...
    element,
    1,
    abi.encode(['string', 'string'], [additionalUri, uri]),
    true
  )
}

export async function changeVotingRules(
  { context, ipfsHttpClient },
  element,
  additionalMetadata,
  quorumPercentage,
...
export async function proposeTransfer(
  ...
  var values = list.map((it) => {
    var val = [
      (it.token.mainInterface && it.token.mainInterface.options.address) ||
        it.token.address,
      [],
      [it.value],
      it.address,
  ...
  await propose(
    ...
    presetNumber,
    abi.encode(
      ['string', 'address', `tuple(${types.join(',')})[]`],
      [additionalUri, treasuryManagerAddress, values]
    )
  )
}
...
export async function proposeVote(
  ...
  var payload = abi.encode(
    ['uint256', 'uint256', 'uint256', 'bool', 'bool', 'string'],
    [
      data.objectId,
      data.accept ? data.value : '0',
      data.accept ? '0' : data.value,
      data.vote,
      data.afterTermination,
      additionalUri,
    ]
  )
  payload = abi.encode(
...
export async function retrieveDelegationProposals(
  { context, web3, account, chainId, getGlobalContract, newContract },
  organization
) {
  var delegationManagers = await getDelegationsManagers(
    { context, web3, account, chainId, getGlobalContract, newContract },
    organization.address
  )

  var proposals = await Promise.all(
    delegationManagers.map(async (it) => {
      var supportedTokenData = await blockchainCall(
        it.delegationsManager.methods.supportedToken
      )
      var proposal = {
        ...it.organization,
        ...
        delegationsManager: {
          ...it,
          supportedTokenData,
          supportedToken: await loadToken(
            { context, chainId, web3, account, newContract, getGlobalContract },
            supportedTokenData[0],
            supportedTokenData[1]
          ),
        },
...
export async function refreshWrappedToken(
  { context, web3, account, chainId, getGlobalContract, newContract },
  element
) {
  if (!element.delegationsManager.wrappedTokenData) {
    var wrappedTokenData = await blockchainCall(
      element.organization.components.delegationTokensManager.contract.methods
        .wrapped,
      element.delegationsManager.supportedTokenData[0],
      element.delegationsManager.supportedTokenData[1],
      element.delegationsManager.delegationsManagerAddress
    )
    wrappedTokenData[1] !== '0' &&
      (element.delegationsManager.wrappedTokenData = wrappedTokenData)
    wrappedTokenData[1] !== '0' &&
      (element.delegationsManager.wrappedToken = await loadToken(
        { context, chainId, web3, account, newContract, getGlobalContract },
        wrappedTokenData[0],
        wrappedTokenData[1]
      ))
...
export async function refreshProposals(
  ...
  var args = {
    ...
    topics: [
      ...
      element.delegationsManager
        ? [abi.encode(['uint256'], [3]), abi.encode(['uint256'], [4])]
        : [
  ...
  var proposals = await retrieveProposals(
    element.organization.components.proposalsManager,
    proposalsInfo.map((it) => it.id)
  )

  var proposalsConfiguration

  proposals = await Promise.all(
    proposals.map(async (proposalData, i) => {
      var proposalInfo = proposalsInfo[i]
      var proposalId = proposalInfo.id
      var modelIndex = proposalInfo.model
      var isOfThisOrganization = await proposalResolvers[modelIndex](
        {
          web3,
          context,
          chainId,
      ...
      proposalsConfiguration =
        ...
        new Promise(async (ok) => {
          var votingTokens = abi.decode(
            ['address[]', 'uint256[]', 'uint256[]'],
            proposalData.votingTokens
          )
          return ok({
            ...element.proposalsConfiguration,
            votingTokens: await Promise.all(
              votingTokens[0].map((_, i) =>
                decodeProposalVotingToken(
                  { account, web3, context, newContract },
                  '0',
                  votingTokens[0][i],
                  votingTokens[1][i].toString(),
                  votingTokens[2][i].toString()
                )
              )
            ),
          })
...
export async function getMetadataByCodeSequence(
  { provider, context },
  codeSequence,
  labelName
) {
  if (codeSequence instanceof Array) {
    return (
      await Promise.all(
        codeSequence.map((it) =>
          getMetadataByCodeSequence({ provider, context }, it)
        )
      )
  ...
  if (!labelName) {
    return (
      await Promise.all(
        ['uri', 'additionalUri'].map((it) =>
          getMetadataByCodeSequence({ provider, context }, codeSequence, it)
        )
      )
    ).reduce((acc, it) => ({ ...acc, ...it }), {})
  }
  ...
  try {
    ...
    item = result
      ? result
      : item.toLowerCase().indexOf('ipfs') === -1
      ? {}
...
const proposalResolvers = {
  0: async function (
    { context, chainId, provider, newContract },
    proposalId,
    proposalData,
    delegationsManager
  ) {
    var address = await getRawField(
      { provider },
    ...
    var metadata = await getMetadataByCodeSequence(
      { provider, context },
      address,
      'uri'
    )
  ...
  1: async function (
    { context, chainId, provider, newContract, element },
    proposalId,
    proposalData,
    delegationsManager
  ) {
    var uri = await getRawField(
      { provider },
      proposalData.codeSequence[0],
      'value'
    )
    ...
    var oldUri = await blockchainCall(
      element.organization.contract.methods.uri,
      { blockNumber: parseInt(proposalData.creationBlock) - 1 }
    )
          ...
          extractRules(
            {
              context,
              provider:
                element.organization.components.proposalsManager.contract
                  .currentProvider,
            },
            model.validatorsAddresses[0],
            model
          ),
          extractRules(
            {
  ...
  3: async function (
    ...
    var treasuryManagerAddress = await getRawField(
      { provider },
      proposalData.codeSequence[0],
  ...
  4: async function (
    { context, newContract, provider, chainId },
    proposalId,
    proposalData,
    delegationsManager
  ) {
    var proposalsManagerAddress = await getRawField(
      { provider },
      proposalData.codeSequence[0],
      'proposalsManagerAddress'
    )
    proposalsManagerAddress = abi.decode(
      ['address'],
      proposalsManagerAddress
    )[0]
    ...
    if (
      delegationsManager.delegationsManagerAddress !== delegationsManagerAddress
    ) {
      return
    }
    ...
    originalProposalData = {
      proposalsManagerAddress,
      proposalsManager: newContract(
        context.ProposalsManagerABI,
        proposalsManagerAddress
      ),
      organizationProposalID: abi
        .decode(['bytes32'], originalProposalData[0])[0]
        .toString(),
      collectionAddress: abi
        .decode(['address'], originalProposalData[1])[0]
        .toString(),
    ...
    var proposalData = (
      await blockchainCall(originalProposalData.proposalsManager.methods.list, [
        originalProposalData.organizationProposalID,
      ])
    )[0]

    var metadata = await getMetadataByCodeSequence(
      {
        provider: originalProposalData.proposalsManager.currentProvider,
        context,
      },
      proposalData.codeSequence
    )
    var codeData = await getData(
      { provider: originalProposalData.proposalsManager.currentProvider },
      proposalData.codeSequence[0]
    )
    originalProposalData = {
    ...
    var votes = fromDecimals(
      originalProposalData.accept !== '0'
        ? originalProposalData.accept
        : originalProposalData.refuse,
      18,
      true
    )
    var symbol = 'OS'
    ...
    if (
      codeData.label === 'setUint256' ||
      codeData.label === 'changeOSInflationRate'
    ) {
      if (!codeData.presetValue) {
        var presetValues =
          wellknownPresets[codeData.uri.split('ipfs://ipfs/').join('')]
            .presetValues
        var cleanedValues = presetValues.map((it) =>
          toDecimals(
            it.split(' OS').join('').split('%').join(''),
            it.indexOf('OS') !== -1 ? 18 : 16
          )
        )
        var presetValue =
          presetValues[cleanedValues.indexOf(codeData.valueUint256)]
        codeData.presetValue = presetValue
      }
      ctx = `${
        originalProposalData.vote ? 'Stake' : 'Unstake'
      } ${votes} ${symbol} in "[${metadata.name}](${getNetworkElement(
        { context, chainId },
        'etherscanURL'
      )}address/${proposalData.codeSequence[0]})" selecting ${
        codeData.presetValue
      }`
    } else {
      ctx = `${
        originalProposalData.vote ? 'Stake' : 'Unstake'
      } ${votes} ${symbol} ${
        originalProposalData.accept !== '0' ? 'in favor of' : 'against'
      } "[${metadata.name}](${getNetworkElement(
        { context, chainId },
        'etherscanURL'
      )}address/${proposalData.codeSequence[0]})"`
    }
...
export async function tryRetrieveDelegationAddressFromItem(
  { context, chainId },
  item
) {
  if (item.delegation) {
  ...
  var host = await getRawField(
    { provider: item.mainInterface.currentProvider },
    delegationTokensManagerAddress,
    'host'
  )
  host = abi.decode(['address'], host)[0].toString()

  var initializer = await getRawField(
    { provider: item.mainInterface.currentProvider },
    host,
    'initializer'
  )
  ...
  if (
    web3Utils.toChecksumAddress(initializer) !==
    web3Utils.toChecksumAddress(
