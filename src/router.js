...
const AppRouter = () => {
  ...
  useEffect(() => {
    if(!window.originalFetch) {
      const urlCacheResolverExluded = [...context.urlCacheResolverExluded, context.urlCacheResolver].map(it => it.toLowerCase())
      ...
      window.fetch = function instrumentedFetch() {
        ...
        if(url.indexOf('myuri') !== -1 || url.indexOf('real_uri') !== -1) {
          return window.localFetch = window.localFetch || new Promise(ok => ok({
            text : () => window.localFetch2 = window.localFetch2 || new Promise(ok2 => ok2("{}")),
            json : () => window.localFetch3 = window.localFetch3 || new Promise(ok3 => ok3({}))
          }))
        }
        var args = [...arguments]
        if(url.indexOf('//') !== -1 && urlCacheResolverExluded.filter(it => url.indexOf(it) !== -1).length === 0) {
          //(args[0] = context.urlCacheResolver + encodeURIComponent(args[0])) && console.log(arguments[0], args[0])
        }
        var result = window.originalFetch.apply(window, [resolveCID(args[0]), ...args.filter((_, i) => i !== 0)])
        return args[0] === arguments[0] ? result : result.then(response => {
    ...
    window.isLocal = window.isLocal || window.location.host.indexOf('127.0.0.1') !== -1
