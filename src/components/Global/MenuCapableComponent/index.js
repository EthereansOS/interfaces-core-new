    ...
    return (
        <div className={style.SectionMinWidth}>
            <div className={className}>
                {!nomenu && menuVoices && !subMenuvoices && menuVoices.filter(it => it.label).length > 1 && 
                    (
                        <>
                        <DappMenu selected={componentIndex} voices={menuVoices}/>
                        <div className={style.ItemsExploreMainTitleArea}>
                            <h2> {menuVoices[componentIndex] && menuVoices[componentIndex].label}
                            ...
                            <p>Lorem ipsum sim dolor  {menuVoices[componentIndex] && menuVoices[componentIndex].label}</p>
                        </div>
                        </>
                    )
                }
                {!nomenu && menuVoices && subMenuvoices && menuVoices.filter(it => it.label).length > 1 && 
                <>
                    <DoubleDappMenu selected={componentIndex} voices={menuVoices} subvoices={menuVoice.subMenuvoices} selectedSubvoice={selectedSubvoice} setSelectedSubVoice={setSelectedSubVoice}/>
                </>
                }
