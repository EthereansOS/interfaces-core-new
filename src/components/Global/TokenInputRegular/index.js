...
const TokenInputRegular = ({
  ...
  useEffect(() => {
    ...
    element &&
      ...
      blockchainCall(element.contract.methods.balanceOf, account).then(
        setBalance
      )
    element &&
      element.id &&
      ...
      !element.l2Address &&
      blockchainCall(
        element.contract.methods.balanceOf,
        account,
        element.id
      ).then(setBalance)
    element &&
      ...
      getRawField(
        ...
        'balanceOf(address)',
        account
      ).then((it) => setBalance(abi.decode(['uint256'], it)[0].toString()))
    ...
    onElement &&
      onElement(
        ...
        (!element
          ...
          : !value || value === '0'
          ? ''
          : toDecimals(value, element.decimals)) || ''
      )
  ...
  const properties = selections.reduce(
    (acc, it) => ({
      ...acc,
      [it]: {
        noETH: noETH === true,
        renderedProperties: { onClick, noBalance },
      },
    }),
    {}
  )
  ...
  return modalIsOpen ? (
    ...
    <div className={style.TradeMarketTokenAll}>
      <div className={style.TradeMarketToken}>
        <a
          onClick={() =>
            onImageClick
              ? onImageClick()
              : (!tokens || tokens.length > 1) && setModalIsOpen(true)
          }
          className={style.TradeMarketTokenSelector}>
          <LogoRenderer input={element} />
          <span>
            {shortenWord({ context, charsAmount: 15 }, element?.symbol || '')}
            {!tokens || tokens.length > 1 ? <span> ▼</span> : ''}
          </span>
        </a>
        {!tokenOnly && (
      ...
      {element && balance !== null && !noBalance && !tokenOnly ? (
        <a
          onClick={() =>
            !disabled && setValue(fromDecimals(balance, element.decimals, true))
          }
          className={style.TradeMarketTokenBalance}>
          Balance: {formatMoney(fromDecimals(balance, element.decimals, true))}{' '}
          {shortenWord({ context, charsAmount: 15 }, element.symbol)}
        </a>
      ) : (
        <a className={style.TradeMarketTokenBalance}>{'\u00a0'}</a>
      )}
    </div>
  )
}
