...
const AttachToOrganization = ({ delegation, element }) => {
  ...
  const { block, chainId, web3, account, newContract, getGlobalContract } =
    useWeb3()
  ...
  useEffect(() => {
    setTimeout(async () => {
      var ai = await blockchainCall(
        element.delegationsManager.methods.attachInsurance
      )
      setAttachInsurance(ai)
      var t = await blockchainCall(
        element.delegationsManager.methods.supportedToken
      )
  ...
  function refreshPaidFor() {
    blockchainCall(
      element.delegationsManager.methods.paidFor,
      delegation.address,
      account
    ).then(setPaidFor)
  }
  ...
  function retirePayment() {
    return blockchainCall(
      element.delegationsManager.methods.retirePayment,
      delegation.address,
      account,
      '0x'
    )
  ...
  function payFor() {
    ...
    if (tokenIsItem) {
      const data = abi.encode(
        ['address', 'address'],
        [delegation.address, account]
      )
      return blockchainCall(
        token.mainInterface.methods.safeTransferFrom,
        account,
        element.delegationsManagerAddress,
        token.id,
        value,
        data
      )
    ...
    return blockchainCall(
      element.delegationsManager.methods.payFor,
      delegation.address,
      value,
      '0x',
      account,
  ...
  const percentage = !paidFor
    ? 0
    : (parseInt(paidFor.totalPaid) / parseInt(attachInsurance)) * 100
  ...
  return (
    <>
      ...
      {opened && (
        <RegularModal close={() => setOpened(false)}>
            ...
            {token && (
              <>
                <div className={style.Upshot}>
                  <span>
                    Staked:{' '}
                    {fromDecimals(paidFor.totalPaid, token.decimals, true)} $
                    {token.symbol}
                  </span>
                  <div className={style.UpshotBar}>
                    <figure style={{ width: readPercentage + '%' }}>
                      {formatMoney(percentage, 4) + '%'}
                    </figure>
                  </div>
                </div>
                <p>
                  Requested Insurance: <br></br>
                  {fromDecimals(attachInsurance, token.decimals, true)} $
                  {token.symbol}
                </p>
                <div>
                  {percentage < 100 && (
                    <>
                      <TokenInputRegular
                        onElement={(token, balance, value) =>
                          setTokenElement({ token, balance, value })
                        }
                        tokens={[token]}
                        selected={token}
                      />
                      <ActionAWeb3Buttons
                        ...
                        balance={tokenElement.balance}
                        value={tokenElement.value}
                        other={element.delegationsManagerAddress}
                        onSuccess={refreshPaidFor}
                        onClick={payFor}
                        noApproveNeeded={tokenIsItem}
                        buttonText="Stake"
                      />
                    </>
                  )}
                  {paidFor.retrieverPaid !== '0' &&
                    `You deposited ${fromDecimals(
                      paidFor.retrieverPaid,
                      token.decimals,
                      true
                    )} $${token.symbol}`}
                  {paidFor.retrieverPaid !== '0' && (
        ...
        <div className={style.GovCardHead}>
          <div className={style.GovCardHeadDelegation}>
            ...
            <div className={style.DelegationWalletsCardBTN}>
              <RegularButtonDuo onClick={() => setOpened(!opened)}>
                {opened ? 'Close' : 'Open'}
              </RegularButtonDuo>
            </div>
          </div>
            ...
            <div className={style.OrganizationsExploreMain}>
              <div className={style.OrgAllSingle}>
                <Web3DependantList
                  ...
                  custom="delegation-grants"
                  renderedProperties={{ delegation: element }}
                  provider={() =>
                    list ||
                    getAvailableDelegationsManagers(
                      {
                        context,
                        chainId,
                        web3,
