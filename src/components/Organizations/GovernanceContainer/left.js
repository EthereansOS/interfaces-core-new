...
const WrapWithdrawModal = ({ selectedToken, tokenSymbol, close, title, onClick, other, noApproveNeeded }) => {
  ...
  const [token, setToken] = useState({ token : selectedToken, balance : '0', value : '0'})
  ...
  return (<RegularModal close={close}>
    <h4>{title} {tokenSymbol}</h4>
    <TokenInputRegular tokens={[token.token]} selected={token.token} onElement={onToken}/>
    <ActionAWeb3Buttons noApproveNeeded={noApproveNeeded} token={token.token} balance={token.balance} value={token.value} buttonText={title} other={other} onSuccess={close} onClick={internalOnClick} />
  </RegularModal>)
}
...
const ProposeDelegationTransfer = ({ element, setOnClick, stateProvider }) => {
  ...
  function disabled() {
    ...
    for(var element of state.list) {
      ...
      if(!element.value || !element.value === '0' || isNaN(parseFloat(element.value))) {
        return true
      }
    }
  ...
  return (<>
    <h4>Transfer - 1/2</h4>
    <p><br/><b>PLEASE NOTE</b>: for security reasons, new Delegations can move a maximum of 70% of their assets for each proposal.</p>
    ...
    <div className={style.TranferETHProp}>
      ...
      <div>
        <a className={style.PlusWallet} onClick={add}>+</a>
      </div>
    </div>
    <RegularButtonDuo onClick={!isDisabled && next} className={isDisabled ? 'Disabled' : undefined}>Next</RegularButtonDuo>
  </>)
}
...
const ProposeDelegationVote = ({ element, setOnClick, stateProvider }) => {
  ...
  useEffect(() => {
    setTimeout(async function() {
      setTotalSupply(await blockchainCall(element.delegationsManager.wrappedToken.mainInterface.methods.totalSupply, element.delegationsManager.wrappedToken.id))
    })
  }, [])
  ...
  useEffect(() => {
    ...
    var value = numberToString((parseFloat(state.percentage) / 100) * parseFloat(totalSupply))
  ...
  function next() {
    ...
    setOnClick(() => additionalMetadata => proposeVote({ context, ipfsHttpClient : useWeb3Data.ipfsHttpClient }, element, additionalMetadata, data))
  }
  ...
  return (<>
      {state.selectProposalModal && <RegularModal close={() => setState(oldValue => ({...oldValue, selectProposalModal: false}))}>
        <div className={style.MinimalGovernanceContainer}>
          <GovernanceContainer element={element.delegationsManager.organization} forDelegationVote={forDelegationVote}/>
        </div>
      </RegularModal>}
      <h4>Vote as a Delegation</h4>
      <div className={style.SelectProposal}>
        <div className={style.SelectProposal}>
          <span>{state.selectedProposal ? state.selectedProposal.name : "Select a governance proposal of the Organization to propose to vote on as a delegation, using the Organization’s governance tokens staked in your delegation."}</span>
        </div>
        <div>
  ...
  return (
    <div className={style.GovLeft}>
      ...
      {wrapWithdrawModal && <WrapWithdrawModal
          ...
          selectedToken={element.delegationsManager[`${wrapWithdrawModal === 'Wrap' ? 'supported' : 'wrapped'}Token`]}
          onClick={token => modalOnClick[wrapWithdrawModal]({...useWeb3Data, element, context }, token)}
      />}

      {proposalModal && <RegularModal close={() => setProposalModal()}>
        <ProposalMetadata {...{
          onSuccess() {
            setProposalModal();
          ...
          element,
          Component: proposalModal === 'Transfer' ? ProposeDelegationTransfer : ProposeDelegationVote
        }}/>
      </RegularModal>}
      ...
      {type === 'delegation' && element.delegationsManager && element.delegationsManager.wrappedToken && element.organization.host === useWeb3Data.account && <>
      <div className={style.HostToolsDelegations}>
        <b>Host Tools</b>
        <RegularButtonDuo onClick={() => setProposalModal("Transfer")}>Propose Transfer</RegularButtonDuo>
        <RegularButtonDuo onClick={() => setProposalModal("Vote")}>Propose Vote</RegularButtonDuo>
      </div>
      </>}
      ...
      {type === 'delegation' && element.delegationsManager && <>
        <div className={style.DelegationWrap}>
          <div className={style.DelegationWrapBox}>
            <p>{!element.delegationsManager.wrappedToken ? "Be the first to " : ""}Stake your {element.delegationsManager.supportedToken.symbol} to receive this Delegation’s governance tokens..</p>
            <RegularButtonDuo onClick={() => setWrapWithdrawModal("Wrap")}>Wrap</RegularButtonDuo>
          </div>
          {element.delegationsManager.wrappedToken && <div className={style.DelegationWrapBox}>
            <p>Unwrap your {element.delegationsManager.supportedToken.symbol} to receive it back and return the Delegation’s governance tokens.</p>
            <RegularButtonDuo onClick={() => setWrapWithdrawModal("Withdraw")}>Unwrap</RegularButtonDuo>
          </div>}
        </div>
      </>}

      {(element?.delegationsManager?.wrappedToken || element.organization.proposalsConfiguration.votingTokens.length > 0) && <div className={style.VotingPowersList}>
        <p><b>Voting Powers:</b></p>
        <VotingPowersList votingTokens={element?.delegationsManager?.wrappedToken ? [{...element.delegationsManager.wrappedToken, weight : 1}] : element.organization.proposalsConfiguration.votingTokens}/>
      </div>}
      {proposalType !== 'poll' && (type !== 'delegation' || element.delegationsManager) && <div className={style.Rules}>
