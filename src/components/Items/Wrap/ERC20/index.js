      ...
      <div className={style.CreationPageLabel}>
        <label>
          ...
          <TokenInputRegular
            ...
            onElement={onElement}
            onlySelections={['ERC-20', 'Items V1']}
          />
          {data && data.token && (
            <ActionAWeb3Buttons
              onSuccess={(transaction) =>
                void (setSuccess(transaction), onSuccess && onSuccess())
              }
              token={data.token}
              other={w20}
              ...
              value={data.value}
              buttonText="Wrap"
              noApproveNeeded={data.token.address === VOID_ETHEREUM_ADDRESS}
              onClick={() =>
                wrapERC20({ web3 }, data.token, data.balance, data.value, w20)
              }
            />
          )}
        </label>
