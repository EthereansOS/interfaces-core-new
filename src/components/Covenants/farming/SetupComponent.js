...
export default (props) => {
  ...
  const { web3, account, chainId, newContract, block, dualChainId, dualBlock } =
    web3Data

  const currentBlock = useMemo(
    () => parseInt(dualBlock || block),
    [block, dualBlock]
  )
  ...
  const flag = useMemo(
    () =>
      web3Utils.toChecksumAddress(element.address) ===
      web3Utils.toChecksumAddress('0x4992e4c2fb8F8aa8aE38bF30ACA69ff24410edf1'),
    [element]
  )
  ...
  const intervalId = useMemo(
    () => new Date().getTime() + '_' + new Date().getTime() * Math.random(),
    []
  )
  ...
  const ethereumAddress = getNetworkElement(
    { context, chainId },
    'wethTokenAddress'
  )
  ...
  const ammAggregatorPromise = useMemo(
    () =>
      new Promise(async (ok) => {
        ...
        const ammAddresses = (await ammAggregator.methods.amms().call()).map(
  ...
  function getFarmingPrestoAddress() {
    var prestoAddress = getNetworkElement(
      { context, chainId },
      'farmingPrestoAddress'
    )
    var oldPrestoAddress = getNetworkElement(
      { context, chainId },
      'farmingPrestoAddressOld'
    )
    var oldFarmingPrestoContracts = getNetworkElement(
      { context, chainId },
      'oldFarmingPrestoContracts'
    ).map((it) => web3Utils.toChecksumAddress(it))
    var lmContractAddress = web3Utils.toChecksumAddress(element.address)
    return oldFarmingPrestoContracts.indexOf(lmContractAddress) === -1
      ? prestoAddress
      : oldPrestoAddress
  }

  const farmingPresto = useMemo(
    () => newContract(context.FarmingPrestoABI, getFarmingPrestoAddress()),
    [chainId]
  )
  ...
  useEffect(
    () =>
      blockchainCall(element.contract.methods.initializer)
        .then((factoryAddress) =>
          blockchainCall(
            newContract(context.EthereansFactoryABI, factoryAddress).methods
              .feeInfo
          )
        )
        .then(setFeeData)
  ...
  useEffect(
    () =>
      ...
      calculateSlippageAmounts(slippage, lpTokenAmount).then(setAmountsMin),
    [element.generation, slippage, lpTokenAmount]
  )
  useEffect(() => {
    if (
      element.generation !== 'gen1' ||
      !tokenAmounts ||
    ...
    const arr = [
      numberToString(
        (parseInt(tokenAmounts[0].full || tokenAmounts[0]) *
          (100 - parseFloat(slippage))) /
          100
      ).split('.')[0],
      numberToString(
        (parseInt(tokenAmounts[1].full || tokenAmounts[1]) *
          (100 - parseFloat(slippage))) /
          100
      ).split('.')[0],
    ]
      ...
      setTimeout(async () => {
        if (element.generation === 'gen1') {
          try {
            ...
            const value = formatNumber(
              fromDecimals(
                lpTokenInfo.tokenValues[tokenIndex],
                lpTokenInfo.tokens[tokenIndex].decimals,
                true
              )
            )
            const price = Object.values(
              (
                await getTokenPricesInDollarsOnCoingecko(
                  { context, web3Data },
                  lpTokenInfo.tokenAddresses[tokenIndex]
                )
              ).data
            )[0]
        ...
        if (element.generation === 'gen2') {
          try {
            ...
            var diluted =
              Math.abs(
                parseInt(setupInfo.tickUpper) - parseInt(setupInfo.tickLower)
              ) >= 180000
            var tickData = {
              diluted,
              ...
              outOfRangeLower:
                parseInt(slot.tick) <= parseInt(setupInfo.tickLower),
              outOfRangeUpper:
                parseInt(slot.tick) >= parseInt(setupInfo.tickUpper),
              tickLowerUSDPrice: 0,
              tickUpperUSDPrice: 0,
              tickCurrentUSDPrice: 0,
            }
            ...
            tickData.cursor = formatMoneyUniV3(
              secondTokenIndex === 1
                ? 100 - tickData.cursorNumber
                : tickData.cursorNumber,
              2
            )

            var tokensForPrice = lpTokenInfo.uniswapTokens.map((it) =>
              it.address === ethereumAddress
                ? VOID_ETHEREUM_ADDRESS
                : it.address
            )
  ...
  useEffect(() => {
    if (
      ...
      !tokenAmounts ||
      tokenAmounts.length === 0 ||
      (tokenAmounts.filter((it) => parseInt(it.full || it) === 0).length ===
        2 &&
    ...
    const index = setupTokens.indexOf(
      setupTokens.filter(
        (it) =>
          ...
          web3Utils.toChecksumAddress(
            isWeth(setupInfo, setupInfo.mainTokenAddress)
              ? VOID_ETHEREUM_ADDRESS
    ...
    if (value < parseInt(setupInfo.minStakeable)) {
      if (minimumStakingError !== undefined && minimumStakingError !== null) {
        minimumStakingErrorTimeout.current &&
          clearTimeout(minimumStakingErrorTimeout.current)
        setMinimumStakingError(
          `${
            setupTokens[index].symbol
          } value has been reset to the minimum amount of ${fromDecimals(
            setupInfo.minStakeable,
            setupTokens[index].decimals
          )}`
        )
        minimumStakingErrorTimeout.current = setTimeout(
          () => setMinimumStakingError(''),
          2500
        )
      } else {
      ...
      return onUpdateTokenAmount(
        fromDecimals(setupInfo.minStakeable, setupTokens[index].decimals, true),
        index
      )
    }
    ...
    feeData &&
      ...
      setTimeout(async () => {
        ...
        const transferOrBurnTypeInApplication =
          feeData.transferOrBurnReceiverInApplication === VOID_ETHEREUM_ADDRESS
            ? 'burn'
            : 'transfer'
        const transferOrBurnTypeInCreation =
          feeData.transferOrBurnReceiverInCreation === VOID_ETHEREUM_ADDRESS
            ? 'burn'
            : 'transfer'
        setFeeData((oldValue) => ({
          ...oldValue,
        ...
        setBurnFeeAllowance(
          parseInt(
            await blockchainCall(
              feeData.tokenToTransferOrBurnInApplication.contract.methods
                .allowance,
              account,
              feeData.operator
            )
  ...
  const recalculateFeeAllowance = useCallback(() => {
    setupTokens &&
      setupTokens.length > 0 &&
      feeData &&
      (feeData.tokenToTransferOrBurnInApplication ||
      ...
      setTimeout(async () => {
        ...
        async function allowed(token, operator, account, amount) {
          ...
          return (
            ...
            parseInt(
              await blockchainCall(
                token.contract.methods.allowance,
                operator,
                account
              )
            )
        ...
        data =
          data[feeData.tokenToTransferOrBurnAddressInApplication.toLowerCase()]
            .usd
        data *= parseFloat(
          fromDecimals(
            feeData.transferOrBurnAmountInApplication,
        ...
        try {
          ...
          const position = await loadFarmingPosition(
            { ...web3Data, context },
            element.contract,
            positionId
          )
          ...
          data = setupTokens.map(
            (it) =>
              data[it.address.toLowerCase()].usd ||
              data[it.address.toLowerCase()] ||
              0
          )
          data = data.map(
            (it, i) =>
              it *
              parseFloat(
                fromDecimals(simulation.fees[i], setupTokens[i].decimals, true)
              )
          )
    ...
    async (farmSetup, farmSetupInfo, reset, loop) => {
      ...
      for (const positionId of positionIds) {
        ...
        if (!positions.filter((it) => it.positionId === positionId)[0]) {
          ...
          loadedSetup = {
            ...loadedSetup,
            setupIndex: pos.setupIndex,
            setupInfo: { ...loadedSetupInfo },
          }
      ...
      setSetupMustBeToggled(
        farmSetup.active &&
          ...
          (parseInt(farmSetup.totalSupply) === 0 ||
            position === undefined ||
            position === null) &&
      ...
      setRewardTokenInfo((oldValue) => ({
        ...
        ...rewardToken,
        approval:
          parseInt(rewardTokenApproval) !== 0 &&
          parseInt(rewardTokenApproval) >= parseInt(rewardToken.balance),
      }))
      ...
      if (!loop) {
        if (element.generation === 'gen1') {
          ...
          const lpTokenApproval = await lpToken.methods
            .allowance(account, element.contract.options.address)
            .call()
          ...
          const amm = amms.filter(
            (it) => it.address === ammAggregatorAddress
          )[0]

          const originalTokenAddresses = info[2].map(
            web3Utils.toChecksumAddress
          )
          const tokenAddresses = originalTokenAddresses.map((it) =>
            farmSetupInfo.involvingETH && it === amm.ethereumAddress
              ? VOID_ETHEREUM_ADDRESS
              : it
          )
          const tokens = await Promise.all(
            tokenAddresses.map((it) =>
              loadTokenFromAddress({ context, ...web3Data, seaport }, it)
            )
          )
          ...
          setLpTokenInfo({
            ...
            approval:
              parseInt(lpTokenApproval) !== 0 &&
              parseInt(lpTokenApproval) >= parseInt(lpTokenBalance),
          })
        } else {
          ...
          var uniswapTokens = await Promise.all(
            [
              ...
              const currentToken = await loadTokenFromAddress(
                { context, ...web3Data, seaport },
                tkAddress
              )
              return new Token(
      ...
      if (!loop) {
        const extensionBalance = await blockchainCall(
          rewardToken.contract.methods.balanceOf,
          element.extensionAddress
        )
      ...
      if (element.generation === 'gen2') {
        try {
          ;({ balances, fees } = await simulateDecreaseLiquidityAndCollect(
            farmSetup.objectId,
            element.address
          ))
        } catch (e) {
          if (farmSetup.totalSupply !== '0') {
            try {
              const lpTokenEthers = new ethers.Contract(
                tokenAddress,
      ...
      if (tokens.length === 0) {
        ...
        setSetupTokens(
          (oldValue) => (oldValue && oldValue.length > 0 && oldValue) || tokens
        )
      }
      ...
      for (const i in tokens) {
        ...
        token.balance = await blockchainCall(
          token.contract.methods.balanceOf,
          account
        )
        token.liquidity = balances[i]
        const approval = await blockchainCall(
          token.contract.methods.allowance,
          account,
          element.address
        )
        approvals.push(
          parseInt(approval) !== 0 &&
            (parseInt(approval) >= parseInt(token.balance) || !token)
        )
        if (
      ...
      if (position) {
        ...
        if (element.generation === 'gen1') {
          const amm = (await ammAggregatorPromise).amms.filter(
            (it) =>
              web3Utils.toChecksumAddress(it.address) ===
              web3Utils.toChecksumAddress(setupInfo.ammPlugin)
          )[0]
        ...
        if (currentBlock < parseInt(farmSetup.endBlock)) {
          freeReward +=
            ...
            (parseInt(position.liquidityPoolTokenAmount) /
              parseInt(farmSetup.totalSupply))
        }
        ...
        var withdrawOnly =
          !farmSetup.active || currentBlock > parseInt(farmSetup.endBlock)
        setManageStatus({
          withdrawOnly,
          additionalFees,
          free,
    ...
    async (noReset, loop) => {
      try {
        ...
        farmSetupInfo = {
          ...farmSetupInfo,
          free: true,
          ethereumAddress: getNetworkElement(
            { context, chainId },
            'wethTokenAddress'
          ),
        }
  ...
  async function simulateDecreaseLiquidityAndCollect(
    objectId,
    lmContractAddress,
    amount
  ) {
    ...
    var bytes = [
      nftPosMan.methods
        .decreaseLiquidity({
          ...
          liquidity:
            amount ||
            (await blockchainCall(nftPosMan.methods.positions, objectId))
              .liquidity,
          amount0Min: 0,
          amount1Min: 0,
    ...
    var liquidity = web3.eth.abi.decodeParameters(
      ['uint128', 'uint128'],
      result[0]
    )
    var balances = web3.eth.abi.decodeParameters(
      ['uint128', 'uint128'],
      result[1]
    )
    var fees = [
  ...
  const calculateApy = async (
    ...
    try {
      ...
      const wusdAddress =
        getNetworkElement({ context, chainId }, 'WUSDAddress') || ''
      const realRewardTokenAddress = await resolveToken(
        { context, ...web3Data },
        rewardTokenAddress
      )
      const realSetupTokens = await Promise.all(
        setupTokens.map((it) =>
          resolveToken({ context, ...web3Data }, { ...it })
        )
      )
      if (setupInfo.free) {
        const searchTokens = [
          realRewardTokenAddress,
          ...realSetupTokens.map((it) => it.address || it),
        ]
          .filter((it) => it)
          .map(web3Utils.toChecksumAddress)
          .filter((it, i, arr) => arr.indexOf(it) === i)
        const res = await getTokenPricesInDollarsOnCoingecko(
          { context, web3Data },
          searchTokens,
          {
            tickToPrice,
        ...
        const mainTokenContract =
          mainTokenAddress !== VOID_ETHEREUM_ADDRESS
            ? newContract(context.ERC20ABI, mainTokenAddress)
            : null
        const decimals =
          mainTokenAddress !== VOID_ETHEREUM_ADDRESS
            ? await blockchainCall(mainTokenContract.methods.decimals)
            : 18
        ...
        const num =
          ...
          100
        const den =
          parseInt(setupInfo.maxStakeable) *
          10 ** (18 - decimals) *
          mainTokenPriceUsd *
          2
        return num / den
  ...
  const isWeth = (setupInfo, address) => {
    return (
      address.toLowerCase() === ethereumAddress.toLowerCase() &&
  ...
  const activateSetup = async () => {
    ...
    try {
      await blockchainCall(
        element.contract.methods.activateSetup,
        setup.infoIndex
      )
      await reloadData()
    } catch (error) {
  ...
  const onTokenApproval = (index, isLp) => {
    ...
    setTokensApprovals(
      tokensApprovals.map((val, i) => (i === index ? true : val))
    )
    try {
  ...
  async function calculateSlippageAmounts(slippage, liquidity, type) {
    ...
    if (element.generation === 'gen1') {
      ...
      result[0] = numberToString(
        parseInt(result[0]) * (1 - slippage / 100)
      ).split('.')[0]
      result[1] = numberToString(
        parseInt(result[1]) * (1 - slippage / 100)
      ).split('.')[0]
      return result
    }
    try {
      ...
      var { amount0: amount0Min, amount1: amount1Min } =
        partialPosition[`${type || 'mint'}AmountsWithSlippage`](toler)
  ...
  async function createPool(liquidity) {
    ...
    var tick = nearestUsableTick(
      parseInt(slot0.tick),
      TICK_SPACINGS[lpTokenInfo.fee]
    )
  ...
  function onUpdateTokenAmount(value, index, isFull) {
    ...
    const fullValue = isFull
      ? value
      : toDecimals(value, setupTokens[index].decimals)
    tks[index] = {
      value: isFull
        ? fromDecimals(value, setupTokens[index].decimals, true)
        : value,
      full: fullValue,
    }
    //isFull && setTokenAmounts(tks.map(it => it))
    if (
      fromDecimals(fullValue, setupTokens[index].decimals, true) ===
      numberToString(tokenAmounts[index].value || 0)
    ) {
      return
    }
    ...
    updateAmountTimeout.current = setTimeout(async function () {
        ...
        if (element.generation === 'gen1') {
          ...
          tks[0] = {
            value:
              index === 0 && !isFull
                ? value
                : fromDecimals(ams[0], setupTokens[0].decimals, true),
            full: ams[0],
          }
          tks[1] = {
            value:
              index === 1 && !isFull
                ? value
                : fromDecimals(ams[1], setupTokens[1].decimals, true),
            full: ams[1],
          }
        } else {
          tokenAddress =
          ...
          if (setup.objectId && setup.objectId !== '0') {
            ...
            tks[0] = {
              ...
              full: toDecimals(
                numberToString(amount0),
                setupTokens[0].decimals
              ),
            }
            tks[1] = {
              value: index === 1 && !isFull ? value : numberToString(amount1),
              full: toDecimals(
                numberToString(amount1),
                setupTokens[1].decimals
              ),
            }
          } else {
            pos = pos.mintAmounts
      ...
      while (
        ...
        !(await addLiquidity({
          test: true,
          tokenAmounts: tks,
          lpTokenAmount: liquidityPoolAmount,
        }))
      ) {
      ...
      if (parseInt(setup.totalSupply) + parseInt(liquidityPoolAmount) > 0) {
        var val =
          (parseInt(liquidityPoolAmount) *
            6400 *
            parseInt(setup.rewardPerBlock)) /
          (parseInt(setup.totalSupply) + parseInt(liquidityPoolAmount))
        val = numberToString(val).split('.')[0]
        if (!isNaN(val)) {
          setFreeEstimatedReward(
            fromDecimals(val, rewardTokenInfo.decimals, true)
          )
        }
      }
    }, 300)
  ...
  const onUpdateLpTokenAmount = async (value, index, isFull) => {
    ...
    updateAmountTimeout.current = setTimeout(async function () {
      try {
        const fullValue = isFull
          ? value
          : fromDecimals(value, lpTokenInfo.decimals)
        ...
        if (!setupInfo.free) {
          ...
          if (parseInt(ams[mainTokenIndex]) > 0) {
            ...
            setLockedEstimatedReward(
              toDecimals(
                parseInt(reward.relativeRewardPerBlock) *
                  (parseInt(setup.endBlock) - currentBlock),
          ...
          const val =
            (parseInt(fromDecimals(value, parseInt(lpTokenInfo.decimals))) *
              6400 *
              parseInt(setup.rewardPerBlock)) /
            (parseInt(setup.totalSupply) +
              parseInt(fromDecimals(value, parseInt(lpTokenInfo.decimals))))
          if (!isNaN(val)) {
            setFreeEstimatedReward(
              fromDecimals(numberToString(val), rewardTokenInfo.decimals, true)
            )
          }
        }
      } catch (error) {
  ...
  async function addLiquidity(data) {
    ...
    var lpAmount = numberToString(
      (lpTokenAmount && lpTokenAmount.full) || lpTokenAmount
    )
    stake.amount = numberToString(
      stake.amountIsLiquidityPool
        ? lpAmount
        : tokenAmounts[mainTokenIndex].full || tokenAmounts[mainTokenIndex]
    )
    ethTokenValue =
      ethTokenIndex === undefined || ethTokenIndex === null
        ? '0'
        : numberToString(
            tokenAmounts[ethTokenIndex].full || tokenAmounts[ethTokenIndex]
          )
    var value =
      setupInfo.involvingETH && !stake.amountIsLiquidityPool
        ? ethTokenValue
        : '0'

    if (!currentPosition || receiver) {
      await blockchainCall(element.contract.methods.openPosition, stake, {
        value,
      })
    } else if (currentPosition) {
      await blockchainCall(
        element.contract.methods.addLiquidity,
  ...
  const removeLiquidity = async () => {
    ...
    const removedLiquidity =
      removalAmount === 100 || setupInfo.minStakeable !== '0'
        ...
        : numberToString(
            (parseInt(manageStatus.liquidityPoolAmount) * removalAmount) / 100
          ).split('.')[0]
    var amMin = await calculateSlippageAmounts(
      slippage,
      removedLiquidity,
      'burn'
    )
    ...
    try {
      ...
      if (
        ...
        message.indexOf('price slippage') === -1
      ) {
        await blockchainCall(
          element.contract.methods.withdrawLiquidity,
          currentPosition.positionId,
  ...
  async function withdrawReward() {
    ...
    try {
      ...
      if (
        ...
        msg.indexOf('allowance') === -1
      ) {
        await blockchainCall(
          element.contract.methods['withdrawReward(uint256)'],
          currentPosition.positionId
        )
      } else {
        throw e
  ...
  const transferPosition = async (positionId, index) => {
    ...
    if (setupInfo.free) {
      ...
      try {
        ...
        await blockchainCall(
          element.contract.methods.transferPosition,
          account,
          positionId
        )
        await reloadData()
  ...
  const approveFeeButton = useMemo(() => {
    if (!burnFeeAllowance) {
      return (
        <ActionAWeb3Button onSuccess={reloadData} onClick={withdrawReward}>
          Claim
        </ActionAWeb3Button>
      )
    }
    return (
      <ApproveButton
        ...
        value={feeData && feeData.transferOrBurnAmountInApplication}
      />
    )
  ...
  const updateEthAmount = async (amount) => {
    try {
      ...
      var ammEthereumAddress = (
        await blockchainCall(ammContract.methods.data)
      )[0]
      ...
      var tokens = await blockchainCall(
        ammContract.methods.byLiquidityPool,
        liquidityPool
      )
      var token0 = newContract(context.ERC20ABI, tokens[2][0])
      var token1 = newContract(context.ERC20ABI, tokens[2][1])
      var token0decimals =
        tokens[2][0] === VOID_ETHEREUM_ADDRESS
          ? 18
          : await blockchainCall(token0.methods.decimals)
      var token1decimals =
        tokens[2][1] === VOID_ETHEREUM_ADDRESS
          ? 18
          : await blockchainCall(token1.methods.decimals)

      var lpDecimals = await blockchainCall(
        newContract(context.ERC20ABI, liquidityPool).methods.decimals
      )
      ...
      async function calculateBestLP(
        firstToken,
        secondToken,
        firstDecimals,
        secondDecimals,
        ...
        var data = await blockchainCall(amm.methods.byTokens, [
          ethereumAddress,
          firstToken,
        ])
        ...
        var mainAmount = formatNumber(
          normalizeValue(data[1][mainTokenIndex], firstDecimals)
        )
        var middleTokenAmount = formatNumber(
          normalizeValue(data[1][middleTokenIndex], 18)
        )

        var constant = mainAmount * middleTokenAmount

        var newMiddleTokenAmount =
          middleTokenAmount + formatNumber(normalizeValue(halfValue, 18))
        ...
        if (!hf) {
          return await calculateBestLP(
            ...
            firstDecimals,
            secondDecimals,
            (halfValue = numberToString(formatNumber(halfValue) * ratio).split(
              '.'
            )[0])
          )
        }
        ...
        token1Value = numberToString(parseInt(token1Value) / ratio).split(
          '.'
        )[0]

        const updatedFirstTokenAmount = formatNumber(
          normalizeValue(token0Value, firstDecimals)
        )
        const updatedSecondTokenAmount = formatNumber(
          normalizeValue(token1Value, secondDecimals)
        )

        liquidityPoolAddress = (
          await blockchainCall(amm.methods.byTokens, [
            ethereumAddress,
            secondToken,
          ])
        )[2]
      ...
      var token0EthLiquidityPoolAddress =
        bestLP.firstTokenEthLiquidityPoolAddress
      var token1EthLiquidityPoolAddress =
        bestLP.secondTokenEthLiquidityPoolAddress
      ...
      token0EthLiquidityPoolAddress !== VOID_ETHEREUM_ADDRESS &&
        (ethValue = web3Utils
          .toBN(ethValue)
          .add(web3Utils.toBN(firstTokenETH))
          .toString())
      token1EthLiquidityPoolAddress !== VOID_ETHEREUM_ADDRESS &&
        (ethValue = web3Utils
          .toBN(ethValue)
          .add(web3Utils.toBN(secondTokenETH))
          .toString())
      info.involvingETH &&
        token0.options.address === ammEthereumAddress &&
        (ethValue = web3Utils
          .toBN(ethValue)
          .add(web3Utils.toBN(firstTokenAmount))
          .toString())
      info.involvingETH &&
        token1.options.address === ammEthereumAddress &&
        (ethValue = web3Utils
          .toBN(ethValue)
          .add(web3Utils.toBN(secondTokenAmount))
          .toString())
      ...
      setLpTokenAmount({
        full: lpAmount,
        value: fromDecimals(lpAmount, lpDecimals),
      })
    } catch (error) {
      console.log(error)
    }
  ...
  const calculateLockedFixedValue = () => {
    ...
    const normalizedRewardPerBlock =
      parseInt(rewardPerBlock) * 10 ** (18 - rewardTokenInfo.decimals)
    const normalizedMaxStakeable =
      parseInt(maxStakeable) * 10 ** (18 - mainTokenInfo.decimals)
    ...
    return canActivateSetup
      ...
      ? 0
      : formatMoneyUniV3(
          amount * (parseInt(setup.endBlock) - parseInt(currentBlock)),
          6
        )
  }
  ...
  function calculateDailyEarnings() {
    ...
    var rewardPerBlock = formatNumber(
      fromDecimals(setup.rewardPerBlock, rewardTokenInfo.decimals, true)
    )
    ...
    var totalSupply = formatNumber(
      fromDecimals(setup.totalSupply, rewardTokenInfo.decimals, true)
    )
    var dailyEarnings =
      (rewardPerBlock * 6400 * liquidityPoolAmount) / totalSupply
  ...
  function calculateTransactedFee(percentage, amount, setupToken) {
    amount = numberToString(
      parseFloat(fromDecimals(percentage, 18, true)) * parseInt(amount)
    ).split('.')[0]
    ...
    return (
      <>
        <div className={style.ActionInfoSection}>
          ...
          <a
            className={style.ActionInfoSectionSettings}
                    ...
                    feeData.feePercentageForTransacted && (
                      <span>
                        {calculateTransactedFee(
                          feeData.feePercentageForTransacted,
                          element.generation === 'gen1'
                            ? manageStatus.tokenAmounts[0]
                            : manageStatus.additionalFees[0],
                          setupTokens[0]
                        )}{' '}
                        ...
                        {calculateTransactedFee(
                          feeData.feePercentageForTransacted,
                          element.generation === 'gen1'
                            ? manageStatus.tokenAmounts[1]
                            : manageStatus.additionalFees[1],
                          setupTokens[1]
                        )}{' '}
  ...
  const getManageAdvanced = () => {
    const RemoveButton = (
      <ActionAWeb3Button onSuccess={reloadData} onClick={removeLiquidity}>
        Remove
      </ActionAWeb3Button>
    )
      ...
      return (
        <div>
                ...
                {manageStatus.tokens.map((token, i) => (
                  <span key={token.address}>
                    ...
                    {formatMoneyUniV3(
                      fromDecimals(
                        (parseInt(
                          manageStatus.tokenAmounts[i].full ||
                            manageStatus.tokenAmounts[i]
                        ) *
                          removalAmount) /
                          100,
                        token.decimals,
          ...
          <div className={style.LiqiudityAffairs}>
            ...
            {setupInfo &&
              setupInfo.minStakeable !== '0' &&
              renderSettings(true, true, undefined, RemoveButton)}
          </div>
        </div>
      )
    }
    ...
    return (
      <div className={style.FarmingDynamics}>
        {setupTokens.map((setupToken, i) => {
          return (
            <div className={style.FarmToken} key={setupToken.address}>
              {(i === 1
                ? tickData.cursorNumber !== 100
                : tickData.cursorNumber !== 0) && (
                <TokenInputRegular
                  ...
                  onElement={(t, _, value) =>
                    onUpdateTokenAmount(
                      fromDecimals(value, t.decimals, true),
                      i
                    )
                  }
                  outputValue={
                    ...
                    fromDecimals(
                      tokenAmounts[i].full || tokenAmounts[i],
                      setupToken.decimals,
                      true
                    )
                  }
        ...
        <div className={style.WizardFooter}>
          ...
          <ActionAWeb3Button
            onSuccess={flag ? undefined : reloadData}
            onClick={flag ? undefined : addLiquidity}
            disabled={
              flag ||
              tokensApprovals.some((value) => !value) ||
              tokenAmounts.some((value) => value === 0)
            }>
            Add Liquidity
          </ActionAWeb3Button>
        </div>
  ...
  return (
    <div className={style.FarmSetupV3}>
        ...
        <div className={style.SetupFarmingInstructions}>
          <div className={style.SetupFarmingInstructionsV3}>
            {setupTokens.map((token, i) => (
              <div key={token.address} className={style.TokenFarmV3InfoBox}>
                {token.address !== VOID_ETHEREUM_ADDRESS ? (
                  <a
                    target="_blank"
            ...
            {!endBlockReached && (
              <p className={style.BlockInfoV3B}>
                {setup.active && parseInt(setup.endBlock) > currentBlock && (
                  <span className={style.V3FarmStatusYEP}>Active</span>
                )}
                {!delayedBlock && (
                  <>
                    {' '}
                    {!setup.active && canActivateSetup ? (
                      <span>{setupReady ? 'new' : 'Soon'}</span>
                    ) : !setup.active ? (
                      <span>Inactive</span>
                    ) : (
                      <></>
                    ...
                    {parseInt(setup.endBlock) <= currentBlock &&
                      parseInt(setup.endBlock) !== 0 && <span>Ended</span>}
                  </>
                )}
                {delayedBlock !== 0 && <span>Soon</span>}
                {!isNaN(apy) && (
                  <>
                    {' '}
                    <b>APR</b>: {apy === 0 ? 0 : formatMoneyUniV3(apy, 3)}%
                  </>
                )}
              </p>
            )}
            {rewardTokenInfo && (
              <p className={style.BlockInfoV3}>
                <b>Daily Rate</b>:{' '}
                {formatMoneyUniV3(
                  fromDecimals(
                    parseInt(setup.rewardPerBlock) * 6400,
                    rewardTokenInfo.decimals,
                    true
                  ),
                  4
            ...
            {mainTokenInfo && setupInfo && setupInfo.minStakeable !== '0' && (
              <p className={style.BlockInfoV3}>
                <b>Minimum Staking</b>:{' '}
                {formatMoneyUniV3(
                  fromDecimals(
                    setupInfo.minStakeable,
                    mainTokenInfo.decimals,
                    true
                  ),
                  6
                )}{' '}
            ...
            {parseInt(setup.endBlock) > 0 ? (
              <p className={style.BlockInfoV3}>
                ...
                <a
                  target="_blank"
                  href={`${getNetworkElement(
                    { context, chainId: dualChainId || chainId },
                    'etherscanURL'
              ...
              <p className={style.BlockInfoV3}>
                <b>Duration</b>:{' '}
                {getPeriodFromDuration(setupInfo.blockDuration)}
              </p>
            )}
            {setupMustBeToggled && (
              <ActionAWeb3Button onSuccess={refresh} onClick={toggleSetup}>
                Toggle
              </ActionAWeb3Button>
            )}
            {!currentPosition &&
              !open &&
              parseInt(setup.endBlock) > parseInt(currentBlock) && (
                <a
                  className={style.RegularButtonDuo}
                  onClick={() =>
                    void (setOpen(true), setWithdrawOpen(false), setEdit(false))
                  }>
                  Farm
                </a>
              )}
            {!currentPosition && open && (
              <RegularButtonDuo
                className={style.RegularButtonDuoBack}
                onClick={() =>
                  void (setOpen(false), setWithdrawOpen(false), setEdit(false))
                }>
                Close
              </RegularButtonDuo>
            )}
            {!delayedBlock && canActivateSetup && (
              <>
                  ...
                  <>
                    {activateLoading ? (
                      ...
                      <>
                        ...
                        <p className={style.FarmDisclamerGas}>
                          You're the first farmer on this setup, you'll spend
                          more gas, but all of the initial currentBlock rewards
                          are yours!{' '}
                        </p>
                      </>
                    )}
                ...
                {!setupReady && (
                  <p>Not ready to be activated, come back at another time</p>
                )}
              </>
            )}
            {delayedBlock !== 0 && (
          ...
          <div className={style.farmInfoCurve}>
            <p className={style.farmInfoCurveL}>
              <p className={style.MAinTokensel}>
                <a
                  onClick={() =>
                    setsecondTokenIndex(1 - (secondTokenIndex || 0))
                  }>
                ...
                {setupTokens[secondTokenIndex].symbol} per{' '}
                {setupTokens[1 - secondTokenIndex].symbol}
              </p>
            </p>
            {element.generation === 'gen1' && lpTokenInfo && (
                ...
                <p className={style.PriceRangeInfoFarm}>
                  <a
                    ...
                    href={`${getNetworkElement(
                      { context, chainId },
                      'etherscanURL'
                    )}address/${lpTokenInfo.contract.options.address}`}>
                    <span className={style.VersionFarm}>
                      {lpTokenInfo.amm.name}
                    </span>
                  </a>
                </p>
              </p>
              ...
              <p className={style.farmInfoCurveR}>
                <p className={style.PriceRangeInfoFarm}>
                  <a
                    ...
                    {formatMoneyUniV3(
                      numberToString(parseInt(lpTokenInfo.fee) / 10000),
                      '2'
                    )}
                    %
                  </a>
                ...
                <div className={style.UniV3CurveViewCurv}>
                  ...
                  <span className={style.CircleLeftV3CurvePrice}>
                    {tickData.diluted ? (
                      ...
                      `${formatMoneyUniV3(tickData.minPrice)} ${
                        setupTokens[secondTokenIndex].symbol
                      }`
                    )}
                  </span>
                  ...
                  <span className={style.CircleRightV3CurvePrice}>
                    {tickData.diluted ? (
                      ...
                      `${formatMoneyUniV3(tickData.maxPrice)} ${
                        setupTokens[secondTokenIndex].symbol
                      }`
                    )}
                  </span>
                  <div
                    className={style.CircleActualPriceV3}
                    style={{ left: `${tickData.cursor}%` }}>
            ...
            <span className={style.UniV3TVLFIV}>
              ...
              {setupTokens.map((token, index) => (
                <span key={token.address}>
                  {formatMoneyUniV3(
                    fromDecimals(token.liquidity, token.decimals, true),
                    4
                  )}{' '}
                  {token.symbol}
                  {index !== setupTokens.length - 1 ? ' - ' : ''}
                </span>
              ))}
            </span>
          </div>
      ...
      {currentPosition && (
        <div
          className={
            style.YourFarmingPositionsFarmingFarmingFarmingChiFarmaViveComeUnPAsha
          }>
          <div className={style.FarmYou}>
            {manageStatus && (
              <p>
                <b>Your Deposit</b>:<br></br>{' '}
                {manageStatus.tokens.map((token, i) => (
                  <span key={token.address}>
                    {' '}
                    {formatMoneyUniV3(
            ...
            {!endBlockReached && (
              <p>
                <b>Daily Earnings</b>:<br></br> {calculateDailyEarnings()}{' '}
                {rewardTokenInfo.symbol}
              </p>
            )}
            ...
            {!manageStatus?.withdrawOnly && !withdrawOpen && (
              <RegularButtonDuo
                onClick={() =>
                  void (setOpen(false), setWithdrawOpen(true), setEdit(false))
                }>
                Decrease
              </RegularButtonDuo>
            )}
