...
const ViewRoutine = ({ loadedElement, onBack }) => {
  ...
  const { chainId, block, newContract, account, dualChainId, dualBlock } =
    web3Data
  ...
  useEffect(() => {
    ...
    address &&
      ...
      getRoutine({ context, ...web3Data }, address).then(setElement)
  }, [pathname])
  ...
  async function getContractMetadata(force) {
    ...
    try {
      ...
      for (var operation of operations) {
        ...
        operation.inputToken = await loadTokenFromAddress(
          { context, ...web3Data, seaport },
          operation.inputTokenAddress === VOID_ETHEREUM_ADDRESS ||
            (operation.amm &&
              operation.enterInETH &&
              web3Utils.toChecksumAddress(operation.inputTokenAddress) ===
                web3Utils.toChecksumAddress(operation.amm.data[0]))
            ? VOID_ETHEREUM_ADDRESS
            : operation.inputTokenAddress
        )
        for (var swapTokenIndex in operation.swapPath) {
          var swapToken =
            operation.swapPath[(swapTokenIndex = parseInt(swapTokenIndex))]
          var data
          if (
            operation.amm &&
            operation.exitInETH &&
            swapTokenIndex === operation.swapPath.length - 1
          ) {
            data = await loadTokenFromAddress(
              { context, ...web3Data, seaport },
              web3Utils.toChecksumAddress(swapToken) ===
                web3Utils.toChecksumAddress(operation.amm.data[0])
                ? VOID_ETHEREUM_ADDRESS
                : swapToken
            )
      ...
      const period = Object.values(context.blockIntervals).filter(
        (value) => value === entry.blockInterval
      )
      const oneHundred = await contract.methods.ONE_HUNDRED().call()
      const executorReward = formatMoney(
        parseFloat(fromDecimals(entry.callerRewardPercentage, 18, true)) * 100
      )
      ...
      var extensionContract = newContract(
        context.FixedInflationExtensionABI,
        await contract.methods.host().call()
      )
      var active = true
      try {
  ...
  async function execute() {
    ...
    var minInputs = (
      await blockchainCall(element.contract.methods.entry)
    )[1].map(() => 0)
    minInputs = await element.contract.methods
      .executeWithMinAmounts(earnByInput === true, minInputs)
      .call()
    minInputs = minInputs[1].map(
      (it) =>
        numberToString((parseInt(it) * parseFloat(100 - slippage)) / 100).split(
          '.'
        )[0]
    )
    var transactionResult = await blockchainCall(
      element.contract.methods.executeWithMinAmounts,
      earnByInput === true,
      minInputs
    )
    var Executed = abi.decode(
      ['bool'],
      transactionResult.logs.filter(
        (it) => it.topics[0] === web3Utils.sha3('Executed(bool)')
      )[0].data
    )[0]
    if (!Executed) {
        ...
        <div className={style.RutineBox}>
          <div className={style.RutineTitle}>
            <div className={style.RutineBack}>
              {onBack && (
                <a className={style.BackButton} onClick={onBack}>
                  X
                </a>
              )}
            </div>
              ...
              <span className={style.floatRight}>
                {onBack && (
                  <a
                    ...
                    onClick={() =>
                      copyToClipboard(
                        `${window.location.href}/${element.address}`
                      )
                    }>
                    <svg
                ...
                {element.host === account && !element.active && (
                  <ActionAWeb3Button
                    className={style.RegularButtonDuo}
                    onSuccess={() => getContractMetadata(true)}
                    onClick={() =>
                      blockchainCall(element.extension.methods.setActive, true)
                    }>
                    Activate
                  </ActionAWeb3Button>
                )}
          ...
          <div className={style.OperationsBox}>
            {element.operations.map((operation, i) => {
              var amount = fromDecimals(
                operation.inputTokenAmount,
                operation.inputTokenAmountIsPercentage
                  ? '18'
                  : operation.inputToken.decimals,
                true
              )
              amount = !operation.inputTokenAmountIsPercentage
                ? amount
                : parseFloat(amount) * 100
              return (
                <div className={style.RoutineOperation} key={i}>
                  <h6>Operation {i + 1}</h6>
                    ...
                    {operation.ammPlugin !== VOID_ETHEREUM_ADDRESS && (
                      <a
                        ...
                        href={`${getNetworkElement(
                          { context, chainId },
                          'etherscanURL'
                        )}address/${operation.ammPlugin}`}>
                        {operation.amm.info[0]}
                      </a>
                    )}
                    {operation.receivers.map((it, i) => {
                      var percentage =
                        i === operation.receiversPercentages.length
                          ? element.oneHundred
                          : operation.receiversPercentages[i]
                      ...
                      return (
                        <a
                          ...
                          {formatMoney(
                            parseFloat(fromDecimals(percentage, 18, true)) * 100
                          )}
                          % Receiver
                        </a>
                      )
                  ...
                  <div className={style.RoutineOperationSchedule}>
                    <p>
                      <b>
                        {operation.inputTokenAmountIsByMint
                          ? 'Mint '
                          : 'Transfer '}
                      </b>
                      ...
                      <LogoRenderer badge input={operation.inputToken} />{' '}
          ...
          <div className={style.OperationsTrigger}>
            {element.executorReward !== 0 && (
              <h5> {formatMoney(element.executorReward)}% execution reward</h5>
            )}
            {element.executable &&
              element.operations.filter(
                (it) => it.ammPlugin !== VOID_ETHEREUM_ADDRESS
              ).length > 0 && (
                <>
                  <select
                    ...
                    value={earnByInput}
                    onChange={(e) =>
                      setEarnByInput(e.currentTarget.value === 'true')
                    }>
                  ...
                  <div className={style.ActionInfoSection}>
                    <a
                      ...
                      <figure>
                        <img
                          src={`${process.env.PUBLIC_URL}/img/settings.svg`}></img>
                      </figure>
                    </a>
                  </div>
                  {settings && (
          ...
          {element.nextBlock > 0 && (
            <h5>
              ...
              <a
                ...
                target="_blank">
                <b>#{element.nextBlock}</b>
              </a>
            </h5>
          )}
          {element.executable && (
            <ActionAWeb3Button
              onSuccess={getContractMetadata}
              onClick={execute}>
              Execute
            </ActionAWeb3Button>
          )}
      ...
      {edit && (
        <RegularModal close={() => setEdit()}>
          <Create
            ...
            onSuccess={() => void (getContractMetadata(true), setEdit())}
          />
        </RegularModal>
      )}
      {onBack ? (
        ...
        <div className={style.CovenantsMainBox}>{children}</div>
      )}
    </>
  )
}
