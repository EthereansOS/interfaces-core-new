...
export default (props) => {
  ...
  const fixedInflationContractAddress = useMemo(() => {
    ...
    fixedInflationContractAddress = isEthereumAddress(
      fixedInflationContractAddress
    )
      ? fixedInflationContractAddress
      : undefined
  ...
  const [
    fixedInflationExtensionTemplateCode,
    setFixedInflationExtensionTemplateCode,
  ] = useState('')
  const [customExtensionData, setCustomExtensionData] = useState(null)

  useEffect(
    () =>
      setTimeout(async function componentDidMount() {
        setFixedInflationExtensionTemplateCode(
          await (await fetch(FixedInflationExtensionTemplateLocation)).text()
        )
        ...
        var fixedInflationContract = await newContract(
          context.NewFixedInflationABI,
          fixedInflationContractAddress
        )
        setExtensionAddress(
          await blockchainCall(fixedInflationContract.methods.host)
        )
        ...
        clonedEntry.callerRewardPercentage = numberToString(
          parseFloat(
            fromDecimals(clonedEntry.callerRewardPercentage, 18, true)
          ) * 100
        )
        clonedEntry.operations = []
        const ethAddress = web3Utils.toChecksumAddress(
        ...
        for (var operation of entry[1]) {
          ...
          op.inputTokenAddress = web3Utils.toChecksumAddress(
            op.inputTokenAddress
          )

          var inputTokenDecimals = '18'
          var inputTokenSymbol = 'ETH'
          if (
            op.inputTokenAddress !== VOID_ETHEREUM_ADDRESS &&
            op.inputTokenAddress !== ethAddress
          ) {
            try {
              var inputTokenContract = newContract(
                context.IERC20ABI,
                op.inputTokenAddress
              )
              inputTokenDecimals = await blockchainCall(
                inputTokenContract.methods.decimals
              )
              inputTokenSymbol = abi.decode(
                ['string'],
                await getRawField(
          ...
          op.inputToken = {
            address: op.inputTokenAddress,
            symbol: inputTokenSymbol,
            decimals: inputTokenDecimals,
          }
          op.actionType =
            op.ammPlugin === VOID_ETHEREUM_ADDRESS ? 'transfer' : 'swap'
          op.inputTokenMethod = op.inputTokenAmountIsByMint ? 'mint' : 'reserve'
          op.transferType = op.inputTokenAmountIsPercentage
            ? 'percentage'
            : 'amount'
          !op.inputTokenAmountIsPercentage && (op.amount = op.inputTokenAmount)
          op.inputTokenAmountIsPercentage &&
            (op.percentage = numberToString(
              parseFloat(fromDecimals(op.inputTokenAmount, 18, true)) * 100
            ))
          if (op.ammPlugin && op.ammPlugin !== VOID_ETHEREUM_ADDRESS) {
            const ammAggregator = await newContract(
              context.AMMAggregatorABI,
              getNetworkElement({ chainId, context }, 'ammAggregatorAddress')
            )
            ...
            for (var i in op.liquidityPoolAddresses) {
              ...
              for (var z in lpTokensAddresses) {
                ...
                if (currentTokenAddress !== VOID_ETHEREUM_ADDRESS) {
                  const currentToken = await newContract(
                    context.ERC20ABI,
                    currentTokenAddress
                  )
                  const currentTokenSymbol = await blockchainCall(
                    currentToken.methods.symbol
                  )
                  symbols.push(currentTokenSymbol)
                }
                ethAddress === currentTokenAddress &&
                  (symbols[symbols.length - 1] = `ETH`)
              ...
              const pathTokenContract = await newContract(
                context.ERC20ABI,
                address
              )
              const symbol =
                op.amm.info[0] !== 'UniV3'
                  ? await blockchainCall(pathTokenContract.methods.symbol)
                  : 'UniV3'
              const decimals =
                op.amm.info[0] !== 'UniV3'
                  ? await blockchainCall(pathTokenContract.methods.decimals)
                  : '18'
              op.pathTokens.push({
                symbol,
                address,
          ...
          for (i in op.oldReceivers) {
            ...
            var percentage = 0
            i !== op.oldReceivers.length - 1 &&
              (percentage = numberToString(
                parseFloat(fromDecimals(op.receiversPercentages[i], 18, true)) *
                  100
              ))
            ...
            op.receivers.push({
              address,
              percentage:
                i !== op.oldReceivers.length - 1 ? percentage : lastPercentage,
            })
          }
  ...
  function deploy() {
    ...
    if (
      ...
      extensionType !== 'fromSourceCode' &&
      (!isEthereumAddress(extensionAddress) || !payload)
    ) {
      return
    }
    ...
    extensionType === 'address' &&
      sequence.push({
        ...
        async onTransactionReceipt(transactionReceipt, state) {
          var fixedInflationFactoryAddress = (
            await blockchainCall(
          ...
          fixedInflationFactoryAddress =
            fixedInflationFactoryAddress[
              fixedInflationFactoryAddress.length - 1
            ]
          if (
            web3Utils.toChecksumAddress(transactionReceipt.to) !==
            web3Utils.toChecksumAddress(fixedInflationFactoryAddress)
          ) {
            throw 'Wrong transaction'
          }
          var fixedInflationFactory = newContract(
            context.NewFactoryABI,
            fixedInflationFactoryAddress
          )
          var nonce = parseInt(
            await sendAsync(
              fixedInflationFactory.currentProvider,
              'eth_getTransactionCount',
              fixedInflationFactory.options.address,
              web3Utils.toHex(parseInt(transactionReceipt.blockNumber) - 1)
            )
          )
          var extensionAddress =
            '0x' +
            rlphash([fixedInflationFactory.options.address, nonce])
              .toString('hex')
              .substring(24)
        ...
        async onAction(state, element) {
          ...
          var transaction = await blockchainCall(
            fixedInflationFactory.methods.cloneDefaultExtension
          )
      ...
      ...[
        {
          ...
          async onTransactionReceipt(transactionReceipt, state) {
            var fixedInflationAddress = abi.decode(
              ['address'],
            ...
            return {
              fixedInflationAddress,
              elaboratedEntry: elaborateEntry(state.entry),
            }
          },
          async onAction(state, element) {
            ...
            var data =
              '0x' +
              newContract(context.FixedInflationABI)
                .methods.init(
            ...
            var transaction = await blockchainCall(
              fixedInflationFactory.methods.deploy,
              data
            )
        ...
        },
        {
          ...
          async onTransactionReceipt(transactionReceipt, state) {
            var extension = newContract(
              context.FixedInflationExtensionABI,
              state.extensionAddress
            )
    ...
    setActionSequence({
      ...
      onComplete(state) {
        ...
        elaborateRecap(
          state.elaboratedEntry.blockInterval,
          state.elaboratedEntry.operations
        )
      },
    })
  }
  ...
  var deployMethodologies = {
    async address() {
      ...
      fixedInflationFactoryAddress =
        fixedInflationFactoryAddress[fixedInflationFactoryAddress.length - 1]
      var fixedInflationFactory = newContract(
        context.NewFactoryABI,
        fixedInflationFactoryAddress
      )
      ...
      await deployMethodologies.deployedContract(
        fixedInflationExtensionAddress,
        payload
      )
    },
    async fromSourceCode() {
      var sendingOptions = { from: account }
      var method = newContract(customExtensionData.abi).deploy({
        data: customExtensionData.bytecode,
      })
      ...
      await deployMethodologies.deployedContract(
        customFixedInflationExtension.options.address
      )
    },
    async deployedContract(preDeployedContract, builtPayload) {
      ...
      var extension = await newContract(
        context.FixedInflationExtensionABI,
        preDeployedContract || extensionAddress
      )
  ...
  async function elaborateRecap(blockInterval, operations) {
    ...
    try {
      blockIntervalLabel = Object.entries(context.blockIntervals).filter(
        (it) => formatNumber(it[1]) === formatNumber(blockInterval)
      )[0][0]
    } catch (e) {}
    var finalRecap = {
      blockInterval:
        blockIntervalLabel ||
        `${blockInterval} block${formatNumber(blockInterval) > 1 ? 's' : ''}`,
      transfers: 0,
      swaps: 0,
    }
    for (var operation of operations) {
      var address = web3Utils.toChecksumAddress(operation.inputTokenAddress)
      var tokenData = (finalRecap[address] =
        finalRecap[address] ||
        (await loadTokenFromAddress({ context, ...web3Data }, address)))
      finalRecap.transfers +=
        operation.ammPlugin === VOID_ETHEREUM_ADDRESS ? 1 : 0
    ...
    for (var key of Object.keys(finalRecap)) {
      ...
      value.mintPercentage &&
        (value.mintPercentage =
          formatNumber(fromDecimals(value.mintPercentage, 18, true)) * 100)
      value.transferPercentage &&
        (value.transferPercentage =
          formatNumber(fromDecimals(value.transferPercentage, 18, true)) * 100)
      value.mintAmount &&
        (value.mintAmount = fromDecimals(
          value.mintAmount,
          value.decimals,
          true
        ))
      value.transferAmount &&
        (value.transferAmount = fromDecimals(
          value.transferAmount,
          value.decimals,
          true
        ))
  ...
  function elaborateEntry(entry) {
    var elaboratedEntry = {
      ...
      callerRewardPercentage: toDecimals(
        numberToString((entry.callerRewardPercentage || 0) / 100),
        18
      ),
      operations: entry.operations.map((operation) => {
        var receivers = operation.receivers.map((it) => it.address)
        var receiversPercentages = operation.receivers.map((it) =>
          toDecimals(numberToString(it.percentage / 100), 18)
        )
        receiversPercentages.pop()
        return {
          inputTokenAddress:
            operation.enterInETH && operation.amm
              ? operation.amm.ethAddress
              : operation.inputToken.address,
          inputTokenAmount:
            operation.transferType === 'percentage'
              ? toDecimals(
                  numberToString(parseFloat(operation.percentage) / 100),
                  '18'
                )
              : operation.amount,
          inputTokenAmountIsPercentage: operation.transferType === 'percentage',
          inputTokenAmountIsByMint: operation.inputTokenMethod === 'mint',
          ammPlugin: operation.amm
            ? operation.amm.ammContract.options.address
            : VOID_ETHEREUM_ADDRESS,
          liquidityPoolAddresses: operation.pathTokens
            ? operation.pathTokens.map((it) => it.address)
            : [],
          swapPath: operation.pathTokens
            ? operation.pathTokens.map((it) => it.outputTokenAddress)
            : [],
          receivers: receivers,
          receiversPercentages: receiversPercentages,
          enterInETH:
            (operation.enterInETH &&
              operation.amm !== undefined &&
              operation.amm !== null) ||
            false,
  ...
  var steps = [
    [
            ...
            <div className={style.CreationPageLabel}>
              ...
              <label className={style.CreationPageLabelF}>
                <p>
                  ...
                  permissions to manage this Routine Contract.{' '}
              ...
              <select
                ...
                value={extensionType}
                onChange={onExtensionType}>
                <option value="">Host type</option>
                <option value="address">Standard (Address, wallet)</option>
                <option value="deployedContract">
                  Custom (Deployed Contract)
                </option>
                {false && (
                  <option value="fromSourceCode">
                    Custom Extension (Deploy Contract)
                  </option>
                )}
              </select>
              {(extensionType === 'address' ||
                  ...
                  {extensionType === 'address' && (
                    <div className={style.CreationPageLabelF}>
                      ...
                      <input
                        ...
                        onKeyUp={(e) =>
                          setWalletAddress(
                            isEthereumAddress(e.currentTarget.value)
                      ...
                      />
                      <p>
                        The address who has hosting permissions for this
                        contract. The host can manage new setups and ending
                        early existing setups.
                      </p>
                    </div>
                  )}
                  ...
                  {extensionType === 'deployedContract' && (
                    <>
                      ...
                      <div className={style.CreationPageLabelF}>
                        ...
                        <input
                          placeholder="Optional init payload"
                          type="text"
                          defaultValue={payload}
                          onKeyUp={(e) => setPayload(e.currentTarget.value)}
                        />
                      </div>
                    </>
                  )}
      ...
      function () {
        return !(extensionType === 'address'
          ? walletAddress && walletAddress !== VOID_ETHEREUM_ADDRESS
          : extensionType === 'deployedContract'
          ? extensionAddress
          : customExtensionData)
      },
    ...
    if (
      abi.filter(
        (abiEntry) =>
          ...
          abiEntry.name === 'active' &&
          abiEntry.outputs &&
          abiEntry.outputs.length === 1 &&
          abiEntry.outputs[0].type === 'bool'
      ).length === 0
    ) {
      return false
    }
    if (
      abi.filter(
        (abiEntry) =>
          ...
          abiEntry.name === 'deactivationByFailure' &&
          (!abiEntry.outputs || abiEntry.outputs.length === 0) &&
          (!abiEntry.inputs || abiEntry.inputs.length === 0)
      ).length === 0
    ) {
      return false
    }
    if (
      abi.filter(
        (abiEntry) =>
          ...
          abiEntry.stateMutability !== 'view' &&
          abiEntry.stateMutability !== 'pure' &&
          abiEntry.name === 'receiveTokens' &&
          (!abiEntry.outputs || abiEntry.outputs.length === 0) &&
          abiEntry.inputs &&
          abiEntry.inputs.length === 3 &&
          abiEntry.inputs[0].type === 'address[]' &&
          abiEntry.inputs[1].type === 'uint256[]' &&
          abiEntry.inputs[2].type === 'uint256[]'
      ).length === 0
    ) {
      return false
    }
    if (
      abi.filter(
        (abiEntry) =>
          ...
          abiEntry.stateMutability !== 'view' &&
          abiEntry.stateMutability !== 'pure' &&
          abiEntry.name === 'setActive' &&
          (!abiEntry.outputs || abiEntry.outputs.length === 0) &&
          abiEntry.inputs &&
          abiEntry.inputs.length === 1 &&
          abiEntry.inputs[0].type === 'bool'
      ).length === 0
    ) {
      return false
    }
  ...
  async function editEntry() {
    var elaboratedEntry = elaborateEntry(entry)
    var fixedInflationExtension = newContract(
      context.FixedInflationExtensionABI,
      extensionAddress
    )
    await blockchainCall(
      fixedInflationExtension.methods.setEntry,
      elaboratedEntry,
      elaboratedEntry.operations
    )
    ...
    return (
      <>
        ...
        <div className={style.ActionBTNCreateX}>
          ...
          <ActionAWeb3Button onSuccess={onSuccess} onClick={editEntry}>
            Deploy
          </ActionAWeb3Button>
        </div>
      </>
  ...
  function render() {
    return <div className={style.CreatePage}>{steps[step][0]()}</div>
  }
    ...
    return (
      <div className={style.CreatePage}>
        ...
        <div className={style.CreationPageLabel}>
          ...
          {extensionType == 'address' ? (
            <>
              <div className={style.FancyExplanationCreate}>
                <p className={style.BreefRecapB}>
                  Before attempting to execute the FI remember to send the
                  amount of token needed to the FI extension address:
                </p>
                <a
                ...
                <a
                  onClick={() => copyToClipboard(extensionAddress)}
                  className={style.RegularButton}>
                  Copy
                </a>
                <p className={style.BreefRecapB}>
                  The first time the FI will fail the execution due to
                  insufficient funds in the extension address, this contract
                  will be automatically deactivated until editing from the host
                  and reactivation manually.<br></br>For more info about hosting
                  a Fixed Inflation contract:{' '}
                  <a
                    target="_blank"
                    href="https://docs.ethos.wiki/covenants/protocols/inflation">
                    Documentation
                  </a>
                </p>
              </div>
            ...
            <>
              <div className={style.FancyExplanationCreate}>
                ...
                <p className={style.BreefRecapB}>
                  Before attempting to execute the FI you first need to do do
                  all of the actions needed to send the amount of token needed
                  to the FI extension address:
                </p>
                <a
                  className={style.RegularButton}
                  target="_blank"
                ...
                <p className={style.BreefRecapB}>
                  If you rule the extension via a DFO or a DAO, be sure to vote
                  to grant permissions from its Treasury.
                </p>
                <p className={style.BreefRecapB}>
                  The first time the FI will fail the execution due to
                  insufficient funds in the extension address, this contract
          ...
          <div className={style.FancyExplanationCreate}>
            <p className={style.BreefRecapB}>
              Fixed Inflation Contract Address: {fixedInflationAddress}{' '}
              <a
                target="_blank"
                href={`${getNetworkElement(
                  { chainId, context },
                  'etherscanURL'
                )}/address/${fixedInflationAddress}`}>
                (Etherscan)
              </a>
            </p>
            <Link
              className={style.RegularButton}
              to={'/covenants/routines/' + fixedInflationAddress}>
              {process.env.PUBLIC_URL + '#/inflation/' + fixedInflationAddress}
            </Link>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      ...
      {!entry ? (
        ...
        <CreateOrEditFixedInflationEntry
          entry={copy(entry)}
          continue={creationComplete}
          saveEntry={saveEntry}
