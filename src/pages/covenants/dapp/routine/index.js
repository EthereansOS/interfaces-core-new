        ...
        <div className={style.RoutineContent}>
            <div className={style.RoutineContentTitle}>
                ...
                {element.host === account && <a className={style.RegularButtonDuo} onClick={() => setEdit(true)}>Edit</a>}
                 <a onClick={() => setOpened(element)} className={style.RegularButtonDuo}>Open</a>
            </div>
...
const Routines = ({selectedSubvoice}) => {
    ...
    const filter = useCallback(element => {
        if(mode === 'hosted' && web3Utils.toChecksumAddress(element.host) !== web3Utils.toChecksumAddress(account)) {
            return false
        }
        return true
    }, [mode, account])
    ...
    return (<>
        ...
        <div className={style.CovenantsMainBox}>
            ...
            />
            {opened && <ViewRoutine loadedElement={opened} onBack={() => setOpened()}/>}
        </div>
    </>)
}
