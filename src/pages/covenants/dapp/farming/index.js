...
const FarmingCard = (props) => {
  const { element, mode, opened, setOpened, rewardTokenAddress, refresh } =
    props

  if (mode === 'positions' && element.positions) {
    return element.positions.map((it, i) => (
...
const sortOrders = [
  ...
  (first, second) => {
    var a = parseInt(
      first.setups.reduce(
        (acc, val) => acc.ethereansosAdd(val.setupInfo.originalRewardPerBlock),
        '0'
      )
    )
    var b = parseInt(
      second.setups.reduce(
        (acc, val) => acc.ethereansosAdd(val.setupInfo.originalRewardPerBlock),
        '0'
      )
    )
...
const Farming = ({ selectedSubvoice, rewardTokenAddress }) => {
    ...
    (element) => {
      ...
      if (
        ...
        web3Utils.toChecksumAddress(address) !==
          web3Utils.toChecksumAddress(element.rewardTokenAddress)
      ) {
        return false
      }
      if (
        mode === 'explore' &&
        activeOnly &&
        element.setups.filter((it) => it.active || it.canActivateSetup)
          .length === 0
      ) {
        return false
      }
  ...
  function refresh() {
    setDiscriminant(
      mode + '_' + chainId + '_' + account + '_' + new Date().getTime()
    )
  ...
  return (
    <>
      ...
      <div className={style.CovenantsMainBox}>
          ...
          {mode !== 'positions' && (
            <div>
              ...
              <select
                ...
                onChange={(e) => setSortOrderIndex(e.currentTarget.value)}
        ...
        <Web3DependantList
          provider={() =>
            allFarmings({ context, ...web3Data, mode, rewardTokenAddress })
          }
          renderedProperties={{
            refresh,
            mode,
            activeOnly: mode === 'explore' ? activeOnly : false,
            opened,
            setOpened,
            rewardTokenAddress,
          }}
