...
export default (props) => {
  ...
  const [farmingExtensionTemplateCode, setFarmingExtensionTemplateCode] =
    useState('')
  ...
  const genConversion = {
    gen1: {
      ...
      async parseSetup(setup) {
        ...
        const parsedSetup = [
          ...
          !isFree
            ? numberToString(
                fromDecimals(numberToString(setup.maxStakeable)),
                mainTokenDecimals
              )
            : 0,
          setup.renewTimes,
          ...
          isFree
            ? 0
            : fromDecimals(numberToString(parseFloat(setup.penaltyFee) / 100)),
          0,
          0,
        ]
        return parsedSetup
      ...
      getInitArray(
        ...
        return [
          web3Utils.toChecksumAddress(
            (extensionAddress ? extensionAddress : hostDeployedContract) ||
              VOID_ETHEREUM_ADDRESS
          ),
          abi.encode(
            ['bytes', 'address', 'bytes'],
            [
              extensionPayload || extensionInitData || '0x',
              rewardTokenAddress,
              encodedSetups || '0x',
            ]
          ),
    ...
    gen2: {
      ...
      initTypes: ['address', 'bytes', 'address', 'bytes'],
      async parseSetup(setup) {
        const parsedSetup = [
          parseInt(setup.blockDuration),
          parseInt(setup.startBlock),
  ...
  useEffect(
    () =>
      rewardTokenAddress &&
      loadTokenFromAddress({ context, ...web3Data }, rewardTokenAddress).then(
        setSelectedRewardToken
      ),
    [rewardTokenAddress]
  )
  ...
  useEffect(
    () => setByMint(element?.byMint === true),
    [selectedRewardToken, element]
  )
  ...
  useEffect(
    () =>
      ...
      setTimeout(async function () {
        setFarmingSetups(
          await getFarmingSetupInfo({ context, seaport, ...web3Data }, element)
        )
  ...
  async function edit() {
    var setupConfigurations = farmingSetups
      .map((info) => {
        if (
          info.editing === true &&
          info.disable !== true &&
          parseInt(info.initialRewardPerBlock) ===
            parseInt(info.rewardPerBlock || info.originalRewardPerBlock || 0) &&
          parseInt(info.initialRenewTimes) === parseInt(info.renewTimes)
        ) {
          return
        }
        ...
        return {
          ...
          info: {
            ...
            originalRewardPerBlock:
              info.rewardPerBlock || info.originalRewardPerBlock,
            mainTokenAddress: info.mainTokenAddress || info.mainToken.address,
            liquidityPoolTokenAddress:
              info.liquidityPoolTokenAddress || info.liquidityPoolToken.address,
            setupsCount: 0,
            lastSetupIndex: 0,
    ...
    return await blockchainCall(
      element.extensionContract.methods.setFarmingSetups,
      setupConfigurations
    )
  }

  const finishButton = !element ? undefined : (
    <ActionAWeb3Button onSuccess={onEditSuccess} onClick={edit}>
      Deploy
    </ActionAWeb3Button>
  )
  ...
  const removeFarmingSetup = (i) => {
    ...
    const updatedSetups = farmingSetup.editing
      ? [...farmingSetups]
      : farmingSetups.filter((_, index) => index !== i)
    setFarmingSetups(updatedSetups)
  }
  ...
  const initializeDeployData = async () => {
    ...
    try {
      const host =
        ...
        web3Utils.toChecksumAddress(
          selectedHost === 'address' ? hostWalletAddress : hostDeployedContract
        )
      const hasExtension =
      ...
      const data = {
        setups: [],
        rewardTokenAddress: selectedRewardToken.address,
        byMint,
        deployContract,
        host,
        hasExtension,
        extensionInitData: extensionPayload || '',
        treasuryAddress,
      }
      if (generation === 'gen1' && !hostDeployedContract && !deployContract) {
        data.extensionInitData = newContract(context.FarmExtensionGen1ABI)
          .methods.init(byMint, host, treasuryAddress || VOID_ETHEREUM_ADDRESS)
          .encodeABI()
      }
      ...
      for (var i in farmingSetups) {
        ...
        var amountToSend = setup.rewardPerBlock.ethereansosMul(
          setup.blockDuration
        )
        calculatedTotalToSend =
          calculatedTotalToSend.ethereansosAdd(amountToSend)
        cumulativeTotalToSend = cumulativeTotalToSend.ethereansosAdd(
          amountToSend.ethereansosMul(
            formatNumber(setup.renewTimes || '0') === 0 ? 1 : setup.renewTimes
          )
        )
  ...
  function createActionSequence(data) {
    async function getFactory(state) {
      ...
      var factoryAddress = (
        await blockchainCall(
          getGlobalContract('factoryOfFactories').methods.get,
          getNetworkElement({ context, chainId }, 'factoryIndices')[
    ...
    const sequence = [
      {
        label: `Deploy Farming Contract${
          generation === 'gen1' && !hostDeployedContract && !deployContract
            ? ' (cloning Default Extension)'
            : ''
        }`,
        async onTransactionReceipt(transactionReceipt, state) {
          ...
          if (
            web3Utils.toChecksumAddress(transactionReceipt.to) !==
            web3Utils.toChecksumAddress(farmFactory.options.address)
          ) {
            throw 'Wrong transaction'
          }
          var farmMainContractAddress = web3.eth.abi.decodeParameter(
            'address',
          ...
          var retrievedExtensionAddress = await getRawField(
            { provider: web3Data.web3.currentProvider },
            farmMainContractAddress,
            'host'
          )
          retrievedExtensionAddress = abi
            .decode(['address'], retrievedExtensionAddress)[0]
            .toString()
          return {
            farmMainContractAddress,
            ...returnDeployData(
              state,
              retrievedExtensionAddress,
              extensionInitData
            ),
          }
        },
        async onAction(state, element) {
          ...
          const encodedSetups = abi.encode(
            [`tuple(${genConversion[generation].setupInfoTypes.join(',')})[]`],
            [setups]
          )
          const params = genConversion[generation].getInitArray(
            extensionAddress,
            extensionInitData,
            rewardTokenAddress,
            encodedSetups
          ...
          console.log(payload)
          const deployTransaction = await blockchainCall(
            farmFactory.methods.deploy,
            payload
          )
          return this.onTransactionReceipt(
    ...
    function returnDeployData(state, extensionAddress, extensionInitData) {
      const deployData = {
        ...state.deployData,
        extensionAddress,
        extensionInitData: extensionInitData || state.extensionInitData,
      }
      return { deployData }
    }
    ...
    !hostDeployedContract &&
      ...
      sequence.unshift({
        ...
        async onTransactionReceipt(transactionReceipt, state) {
          ...
          var nonce = parseInt(
            await sendAsync(
              ...
              farmFactory.options.address,
              web3Utils.toHex(parseInt(transactionReceipt.blockNumber) - 1)
            )
          )
          var extensionAddress =
            '0x' +
            rlphash([farmFactory.options.address, nonce])
              .toString('hex')
              .substring(24)
          const farmExtension = newContract(
            context[
              generation === 'gen2'
                ? 'FarmExtensionGen2ABI'
                : 'FarmExtensionGen1ABI'
            ],
            extensionAddress
          )
          const extensionInitData = farmExtension.methods
            .init(byMint, host, treasuryAddress || VOID_ETHEREUM_ADDRESS)
            .encodeABI()
        ...
        async onAction(state, element) {
          const farmFactory = await getFactory(state)
          const transaction = await blockchainCall(
            farmFactory.methods.cloneDefaultExtension
          )
    ...
    !hostDeployedContract &&
      ...
      sequence.unshift({
        ...
        async onAction(state, element) {
          ...
          const extension = await deployMethod.send({
            ...payloadData,
            gasLimit,
            gas: gasLimit,
          })
  ...
  const deploy = async () => {
    ...
    try {
      ...
      var factoryAddress = (
        await blockchainCall(
          getGlobalContract('factoryOfFactories').methods.get,
          getNetworkElement({ context, chainId }, 'factoryIndices').farming
        )
      ).factoryList
      factoryAddress = factoryAddress[factoryAddress.length - 1]
      factoryAddress =
      ...
      console.log(payload)
      deployTransaction = await blockchainCall(
        farmFactory.methods.deploy,
        payload
      )
      ...
      !extensionAddress &&
        setDeployData({ ...deployData, extensionAddress: params[0] })
      setFarmingContract(farmMainContractAddress)
    } catch (error) {
      console.log(error)
      error = true
  ...
  const deployExtension = async () => {
    ...
    try {
      ...
      if (!deployContract) {
        ...
        var nonce = parseInt(
          await sendAsync(
            farmFactory.currentProvider,
            'eth_getTransactionCount',
            farmFactory.options.address,
            web3Utils.toHex(parseInt(transaction.blockNumber) - 1)
          )
        )
        var extensionAddress =
          '0x' +
        ...
        if (!extensionAddress) {
          ...
          return setTimeout(() =>
            alert(
              'Error by the connected node while retrieving transaction info, please check your latest transaction on Etherscan and retrieve the deployed contract address located in the "Internal Txns" section (the one at the right side of the green arrow in the table)'
            )
          )
        }
        setDeployData({ ...deployData, extensionAddress, extensionInitData })
        ...
        setDeployData({
          ...deployData,
          extensionAddress: extension.options.address,
        })
      }
    ...
    if (
      abi.filter(
        (abiEntry) =>
          ...
          abiEntry.name === 'transferTo' &&
          (!abiEntry.outputs || abiEntry.outputs.length === 0) &&
          abiEntry.inputs &&
          abiEntry.inputs.length === 1 &&
          abiEntry.inputs[0].type === 'uint256'
      ).length === 0
    ) {
      return false
    }
  ...
  const getCreationComponent = () => {
    return (
      <div className={style.uuuuTokenLoad}>
        <ScrollToTopOnMount />
        ...
        <div className={style.CreationPageLabel}>
          ...
          <label className={style.CreationPageLabelF}>
            <p>
              The reward token is the token chosen to reward farmers and can be
              one per contract.
            </p>
          </label>
          <TokenInputRegular
            tokenOnly
            onElement={setSelectedRewardToken}
            selected={selectedRewardToken}
          />
          {selectedRewardToken &&
            selectedRewardToken.address !== VOID_ETHEREUM_ADDRESS && (
              <div>
                <div className={style.CreationPageLabelF}>
                  <h6>Origin of funds</h6>
                  <select
                    className={style.CreationSelectW}
                    value={byMint === true ? 'true' : 'false'}
                    onChange={(e) => setByMint(e.target.value === 'true')}>
                  ...
                  <p>
                    {' '}
                    If “by reserve” is selected, the input token will be sent
                    from a wallet. If “by mint” is selected, it will be minted
                    and then sent. The logic of this action must be carefully
                    coded into the extension! To learn more, read the{' '}
                    <a
                      target="_blank"
                      href="https://docs.ethos.wiki/covenants/">
                      Documentation
                    </a>
                  </p>
                </div>
  ...
  const getDeployComponent = () => {
    ...
    if (deployStep === 1) {
      ...
      return (
        <div>
          <div>
            <h6>
              <b>Deploy Farming Contract</b>
            </h6>
          </div>
          <div>
            <a onClick={() => setDeployStep(hostDeployedContract ? 0 : 1)}>
              Back
            </a>
    ...
    return (
      <div>
        ...
        <label className={style.CreationPageLabelF}>
          <p>
            The host is the wallet, contract, dApp or Organization with
            permissions to manage this Farming Contract.{' '}
            <a target="_blank" href="https://docs.ethos.wiki/covenants/">
        ...
        {selectedHost === 'address' ? (
          <>
            <div className={style.CreationPageLabelF}>
              ...
              <input
                ...
                onChange={(e) =>
                  setHostWalletAddress(e.target.value.toString())
                }
            ...
            <div className={style.CreationPageLabelF}>
              ...
              {hasTreasuryAddress && (
                <input
                  ...
                  onChange={onTreasuryAddressChange}
          ...
          <>
            <div>
              <p>
                Deploy a custom extension contract. In the IDE, we loaded a
                simple extension contract, and you can use it as a guide. Before
                building a custom contract, we kindly recommend reading the
                Covenants Documentation. Do it at your own risk.
              </p>
            </div>
            <div></div>
            <div>
              ...
              <div>
                <input
                  type="text"
                  value={extensionPayload || ''}
                  onChange={(e) =>
                    setExtensionPayload(e.target.value.toString())
                  }
                  placeholder={'Payload'}
                  aria-label={'Payload'}
                />
          ...
          <>
            <div className={style.CreationPageLabelF}>
              ...
              <input
                type="text"
                value={hostDeployedContract}
                onChange={(e) =>
                  setHostDeployedContract(e.target.value.toString())
                }
                placeholder="Insert extension address"
            ...
            <div className={style.CreationPageLabelF}>
              ...
              <input
                type="text"
                value={extensionPayload || ''}
                onChange={(e) => setExtensionPayload(e.target.value.toString())}
                placeholder={'Payload'}
                aria-label={'Payload'}
              />
            </div>
    ...
    return (
      <div>
        ...
        <div className={style.CreationPageLabel}>
          ...
          <label className={style.CreationPageLabelF}>
            <p>
              Covenants farming contracts are able to manage multiple setups.
              Every setup can reward a single LP with a custom Reward/Block and
              duration.
            </p>
          </label>
    ...
    return (
      <div className={style.CreationPageLabel}>
        ...
        {selectedHost === 'wallet' ? (
          <>
            <div className={style.FancyExplanationCreate}>
              <p className={style.BreefRecapB}>
                Before attempting to activate the contract’s setups,{' '}
                <b>
                  remember to send at least{' '}
                  {fromDecimals(
                    totalRewardToSend,
                ...
                to the extension contract:
              </p>
              <a
                className={style.RegularButton}
                href={
                  getNetworkElement({ context, chainId }, 'etherscanURL') +
                  'address/' +
                  deployData?.extensionAddress
                }
                target="_blank">
                {deployData?.extensionAddress}
              </a>
              <a
                onClick={() => copyToClipboard(deployData?.extensionAddress)}
                className={style.RegularButton}>
                Copy
              </a>
              {/*Calculate total needed taking into acount repet in setups*/}
              <p className={style.BreefRecapB}>
                Taking into account all of the Renewable Setups, the total
                amount of tokens needed, is{' '}
                {fromDecimals(
                  cumulativeRewardToSend,
                  selectedRewardToken?.decimals,
                  true
                )}{' '}
                {selectedRewardToken?.symbol}{' '}
            ...
            {!hasTreasuryAddress && (
              <div className={style.FancyExplanationCreate}>
                {' '}
                <p className={style.BreefRecapB}>
                  Unissued reward tokens will be transferred automagically to
                  the Extension Contract once every farmed position withdraws
                  their liquidity at the end of the setup.
                </p>
              </div>
            )}
            ...
            {hasTreasuryAddress && (
              <>
                <div className={style.FancyExplanationCreate}>
                  <p className={style.BreefRecapB}>
                    Unissued reward tokens will be transferred automagically to
                    the selected treasury address once every farmed position
                    withdraws their liquidity at the end of the setup.
                  </p>
          ...
          <>
            <div className={style.FancyExplanationCreate}>
              ...
              <p className={style.BreefRecapB}>
                Before attempting to activate the contract’s setups,{' '}
                <b>
                  you first need to do do all of the actions needed to send at
                  least{' '}
                  {fromDecimals(
                    totalRewardToSend,
                    selectedRewardToken?.decimals,
              ...
              <p className={style.BreefRecapB}>
                If you rule the extension via a DFO or a DAO, be sure to vote to
                grant permissions from its Treasury.
              </p>
            </div>
          </>
        )}
        <div className={style.FancyExplanationCreate}>
          <p className={style.BreefRecapB}>
            Yuor Farming Contract is available via this link:{' '}
          </p>
          <Link
            className={style.RegularButton}
            to={'/covenants/farming/' + farmingContract}>
            {process.env.PUBLIC_URL + '#/covenants/farming/' + farmingContract}
          </Link>
          <p className={style.BreefRecapB}>
            If you have selected the “Repeat” function for a setup, don’t forget
            to keep track of how many tokens are in the extension. If one cycle
            ends and the extension doesn’t have the required amount of tokens
            for the next, it won’t be able to send them to the contract, and the
            setup will not repeat and instead deactivate. For more information
            on this, read the{' '}
            <a target="_blank" href="https://docs.ethos.wiki/covenants/">
              Documentation
            </a>
      ...
      <div className={style.CreatePage}>
        ...
        <div className={style.CreationPageLabel}>
          <div className={style.SelectGenFarm}>
            ...
            <label className={style.CreationPageLabelF}>
              <p>
                Build a farming contract with multiple customizable setups.
                Covenants farming contracts can be extended and governed by a
                wallet, an automated contract or an Organization.
              </p>
            </label>
            {chainId !== 10 && false && (
              <div className={style.CreateBoxDesc}>
                <h6>Multi AMM</h6>
                <p>
                  Powered by the AMM aggregator, these contracts work with{' '}
                  <b>Uniswap V2, Balancer V1, Mooniswap V1 and Sushiswap V1.</b>
                </p>
                <a
                  className={style.RegularButtonDuo}
                  onClick={() =>
                    void (setGeneration('gen1'), setRegularNFT(false))
                  }>
                  Select
                </a>
              </div>
            ...
            {true && (
              <div className={style.CreateBoxDesc}>
                <h6>Uniswap V3</h6>
                <p>
                  Designed for <b>Uniswap v3</b>, these contracts enable secure
                  farming and customizable price curves.
                </p>
                <button
                  className={style.footerNextButton}
                  onClick={() =>
                    void (setGeneration('gen2'), setRegularNFT(true))
                  }>
                  Select
                </button>
              </div>
