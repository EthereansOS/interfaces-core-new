  ...
  async function terminate() {
    try {
      await blockchainCall(
        element.proposalsManager.methods.terminate,
        element.proposalIds
      )
    } catch (e) {
      alert(e.message || e)
  ...
  return (
    <div>
      ...
      <span>
        End block:{' '}
        <a
          href={'https://rinkeby.etherscan.io/block/' + element.endBlock}
          target="_blank">
          #{element.endBlock}
        </a>
      </span>
...
const VotingToken = ({ proposalId, proposalsManager, element }) => {
  ...
  useEffect(() => {
    async function ask() {
      var x = await blockchainCall(
        proposalsManager.methods.votes,
        [proposalId],
        [account],
        [[element.itemKey]]
      )
  ...
  async function permit() {
    ...
    var value = acc.ethereansosAdd(ref)
    try {
      var permitSignature = await preparePermit(
        { account, chainId },
        contract,
        proposalsManager.options.address,
        value
  ...
  async function vote() {
    ...
    try {
      permitSignatureData = abi.encode(
    ...
    try {
      if (element.passedAsERC20 || element.address === VOID_ETHEREUM_ADDRESS) {
        await blockchainCall(
          ...
          false,
          { value: element.address === VOID_ETHEREUM_ADDRESS ? value : '0' }
        )
      } else {
        var data = abi.encode(
          ['bytes32', 'uint256', 'uint256', 'address', 'bool'],
          [proposalId, acc, ref, address || VOID_ETHEREUM_ADDRESS, false]
        )
        await blockchainCall(
          element.mainInterface.methods.safeTransferFrom,
          account,
          proposalsManager.options.address,
          element.id,
          value,
  ...
  async function withdraw() {
    try {
      await blockchainCall(
        proposalsManager.methods.withdrawAll,
        [proposalId],
        address || VOID_ETHEREUM_ADDRESS
      )
    } catch (e) {
    ...
    <div>
      ...
      <div>
        ...
        <input
          type="number"
          value={acceptsInput}
          onChange={(e) => setAcceptsInput(e.currentTarget.value)}
        />
      </div>
      <div>
        Refuses
        <input
          type="number"
          value={refusesInput}
          onChange={(e) => setRefusesInput(e.currentTarget.value)}
        />
      </div>
      <div>
        Address:
        ...
        getProposal(
          {
            ...
            context,
            newContract,
            contract: getGlobalContract('ballotMaker'),
          },
          id
        )
      }
