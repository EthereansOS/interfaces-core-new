...
const DeckView = () => {
  ...
  const [submenuSelection, setSubmenuSelection] = useState(
    deckSubmenuVoices[1].id
  )
  const [cart, setCart] = useState()
  const [discriminant, setDiscriminant] = useState(new Date().getTime())

  const onCartAction = (inputType, selectedAmount, reserveAll, decimals) =>
    cartAction(
  ...
  useEffect(() => {
    ...
    try {
      ...
      loadDeckItemFromAddress({ context, ...web3Data, seaport }, itemId).then(
        setItem
      )
    } catch (e) {}
  }, [pathname])
      ...
      {item && (
        <>
          ...
          <DappSubMenu
            ...
            voices={deckSubmenuVoices.map((it) => ({
              ...it,
              onClick: () =>
                submenuSelection !== it.id &&
                void (setCart(), setSubmenuSelection(it.id)),
            }))}
          />
          ...
          {cart && cart.length > 0 && (
            <Cart
              ...
              mode={submenuSelection}
              cart={cart}
              onDelete={(i) => setCart((c) => c.splice(i, 1) && [...c])}
              onAction={onCartAction}
              onSecondaryAction={onCartSecondaryAction}
              onSuccess={() =>
                void (setCart(), setDiscriminant(new Date().getTime()))
              }
            />
          )}
        </>
